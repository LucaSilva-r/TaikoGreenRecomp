/* Adapted from Zucchini/TaikoZucchini/core/title_render.c (2026-09-09).
 * Keeps its calibrated native title profiles; uses the host embedded font. */
...
static int font_ready(void) {
    ...
    g_font_state = -1;
    if (FT_Init_FreeType(&g_lib)) return 0;
    ...
    if (FT_New_Memory_Face(g_lib, taiko_overlay_font_data,
                           taiko_overlay_font_size, 0, &g_face)) return 0;
#else
    const char *path = getenv("TAIKO_OVERLAY_FONT");
    if (!path || !*path) path = "fonts/font.ttf";
    if (FT_New_Face(g_lib, path, 0, &g_face)) return 0;
#endif
    if (FT_Set_Pixel_Sizes(g_face, 0, REF_PX)) return 0;
