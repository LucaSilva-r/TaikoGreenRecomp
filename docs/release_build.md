...
Copy `build/TaikoRecomp.exe`, `build/dxcompiler.dll`, `build/dxil.dll`, `build/zlib1.dll`, and the
complete `build/D3D12` directory into the user's dumped game directory:

```text
USRDIR/
  TaikoRecomp.exe
  taiko_config.cfg       # generated by TaikoRecomp.exe on first launch
  zlib1.dll
  dxcompiler.dll
  dxil.dll
  D3D12/
...
The executable embeds `taiko_config.example.cfg` as its current schema and
creates `taiko_config.cfg` beside itself on first launch. When `config_version`
changes it repairs the file, preserving recognized values and dropping stale
keys. This one INI file owns online credentials, the persistent audio offset,
and normal game/audio/video/input/runtime options.
