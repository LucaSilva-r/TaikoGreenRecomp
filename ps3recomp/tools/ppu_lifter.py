...
#define vm_read32  ppu_vm_read32_fast
#define vm_read64  ppu_vm_read64_fast
#define vm_write8  ppu_vm_write8_fast
#define vm_write16 ppu_vm_write16_fast
#define vm_write32 ppu_vm_write32_fast
#define vm_write64 ppu_vm_write64_fast
#endif
...
/* Cross-fragment continuation slots, indexed by guest thread ID. MinGW emits
 * C++ thread_local through __emutls_get_address/pthread_getspecific, and this
 * pointer is checked after almost every lifted call. A cache-line-sized row per
 * guest thread keeps the hot check a plain load without sharing continuations. */
extern "C" void (*g_trampoline_slots[256][8])(void*);
#define g_trampoline_fn (g_trampoline_slots[((unsigned)((ctx)->thread_id)) % 256u][0])
...
class PPULifter:
    ...
    def _translate_op(self, insn: Instruction, func: LiftedFunction) -> str:
        ...
        if mn == "lvebx" or mn == "lvehx" or mn == "lvewx":
            ...
            ra = _reg_idx(ops[1])
            rb = _reg_idx(ops[2])
            # Load one naturally-aligned element into the vector slot selected
            # by EA's low four bits.  The other bytes are unchanged.  This
            # placement is essential for the standard lvsl+lve*x+vperm idiom:
            # putting every element at byte zero makes vperm select zeros for
            # any non-zero EA offset.
            size = {"lvebx": 1, "lvehx": 2, "lvewx": 4}[mn]
            return (f"{{ uint64_t ea = {_xea(ra,rb)}; "
                    f"uint32_t slot = (uint32_t)ea & (16u - {size}u); "
                    f"uint32_t mem = (uint32_t)ea & ~({size}u - 1u); "
                    f"memcpy((uint8_t*)&ctx->vr[{vd}] + slot, vm_base + mem, {size}); }}")
        ...
        if mn == "stvebx" or mn == "stvehx" or mn == "stvewx":
            ...
            return (f"{{ uint64_t ea = {_xea(ra,rb)}; "
                    f"uint32_t slot = (uint32_t)ea & (16u - {size}u); "
                    f"uint32_t mem = (uint32_t)ea & ~({size}u - 1u); "
                    f"memcpy(vm_base + mem, (uint8_t*)&ctx->vr[{vs}] + slot, {size}); }}")
        ...
        if mn.startswith("vmrg"):
            vd, va, vb = int(ops[0][1:]), int(ops[1][1:]), int(ops[2][1:])
            # mn is vmrg + {h,l} + {b,h,w}. The element size is the LAST
            # character; testing `"h" in mn[4:]` also matches the h/l selector,
            # which silently turned vmrghw (merge WORD) into a halfword merge --
            # it kept only the high halfword of each word. vmrglw escaped by
            # luck ("lw" has no 'h'). That broke the 4x4 matrix inverse at
            # func_0051F8EC and, downstream, every bone matrix.
            high = mn[4] == "h"
            size = mn[5]
            if size == "b":
                ty, cnt = "uint8_t", 16
...
def main() -> None:
    ...
    parser.add_argument("--extra-targets", metavar="FILE", default=None,
                        help="JSON list of addresses reached only by computed "
                             "branches (e.g. recovered jump-table cases). They "
                             "join branch_targets, so the mid-function pass "
                             "emits a TAIL-ENTRY WRAPPER for each -- lifting "
                             "from the address to the enclosing function's end, "
                             "sharing its stack frame. Do NOT instead seed these "
                             "into find_functions: that makes each case a real "
                             "function, clipping the parent's extent, and the "
                             "case then runs an epilogue for a frame it never "
                             "built (corrupts r1/LR).")
    ...
    if args.extra_targets:
        with open(args.extra_targets) as _tf:
            _extra = {int(str(a), 0) for a in json.load(_tf)}
        _extra -= {f.start_addr for f in lifter.functions}
        lifter.branch_targets |= _extra
        print(f"  extra-targets: +{len(_extra)} computed-branch targets")
