...
class SPULifter:
    ...
    def lift_function(self, insns: list[SPUInstruction],
        ...
        # A stop-and-signal may be a resumable raw-SPU syscall (notably stop
        # 0x110 receive_event). spu_stop() longjmps for terminal stops and
        # returns for serviced calls, so a returning stop falls through.
        _TERMINATORS = {"br", "bra", "bi", "iret"}
    ...
    def _translate(self, insn: SPUInstruction, func: LiftedFunction) -> str:
        ...
        if mn == "stopd":
            return ("ctx->stop_code = 0u; "
                    "ctx->status = SPU_STATUS_STOPPED_BY_STOP; spu_stop(ctx);")
    ...
    def _cond(self, mn: str, reg: str) -> str:
        ...
        # BRHZ/BRHNZ/IHZ/IHNZ test the halfword in the PREFERRED slot, which the
        # CBEA defines as bytes 2-3 of the register. Our u128 is host-native per
        # 32-bit word (SPU_W reverses bytes within a word: SPU byte 0 -> _u8[3]),
        # so SPU bytes 2-3 are _u16[0], not _u16[1]. Reading _u16[1] tested SPU
        # bytes 0-1 and inverted these branches wherever the two halves differ --
        # e.g. the audio mixer's VAG decoder at 0x2CE4, which then skipped its
        # decode path and emitted silence.
        half = f"ctx->gpr[{reg}]._u16[0]"   # preferred halfword = SPU bytes 2-3
        table = {
...
def main() -> None:
    ...
    if args.extra_funcs:
        ...
        for a in extra:
            ...
            if not split and not any(s == a for s, _ in bset):
                # An indirect-only entry can sit in an uncovered gap between
                # auto-detected basic blocks. Do not extend it to end-of-text:
                # that lifts every later block/data table into one duplicate
                # mega-function. End at the next known block; normal branch
                # emission will tail-call that block when control falls through.
                next_start = min((s for s, _ in bset if s > a),
                                 default=base + len(data))
    ...
    # A resumable stop-and-signal is a raw-SPU syscall (notably stop 0x110,
    # sys_spu_thread_receive_event): the kernel services it and execution
    # continues at the following instruction. Block detection still ends the
    # block at the stop, and the fall-through tail-call is only emitted when
    # that address is also a function start -- so without this the continuation
    # is silently dropped from the lift. For receive_event that continuation is
    # the `rdch SPU_RdInMbox` sequence draining the kernel's reply, and leaving
    # those words unread makes the guest's `rchcnt(SPU_RdInMbox) == 0` guard
    # fail forever, so it never sends its reply back to the PPU.
    by_addr = {i.addr: i for i in insns}
    text_end = base + len(data)
    resumed = []
    for (s, e) in list(bounds):
        li = by_addr.get(e - 4)
        if li is None or li.mnemonic not in ("stop", "stopd"):
            continue
        if e >= text_end or any(s2 == e for s2, _ in bounds):
            continue
        newb = []
        split = False
        for (s2, e2) in bounds:
            if s2 < e < e2:
                newb.append((s2, e)); newb.append((e, e2)); split = True
            else:
                newb.append((s2, e2))
        if not split:
            nxt = min((x for x, _ in bounds if x > e), default=text_end)
            newb.append((e, nxt))
        bounds = sorted(set(newb))
        resumed.append(e)
    if resumed:
        sys.stderr.write("[spu_lifter] resumable-stop continuations: %s\n" %
                         ", ".join("0x%X" % a for a in resumed))
