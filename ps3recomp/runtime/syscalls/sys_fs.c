...
void sys_fs_translate_path(const char* ps3_path, char* host_path, int host_path_size)
{
    ...
    const int usrdir_layout = layout && strcmp(layout, "usrdir") == 0;
    ...
    if (usrdir_layout) {
        ...
        if (strncmp(ps3_path, disc_usrdir, sizeof(disc_usrdir) - 1) == 0)
            rel = ps3_path + sizeof(disc_usrdir) - 1;
        else {
            const char* usr = strstr(ps3_path, "/USRDIR/");
            if (usr) rel = usr + 8;
            else {
                size_t length = strlen(ps3_path);
                if (length >= 7 &&
                    strcmp(ps3_path + length - 7, "/USRDIR") == 0)
                    rel = ps3_path + length;
                else if (ah) rel = ah + 9;
                else if (strcmp(ps3_path, "/app_home") == 0 ||
                         strcmp(ps3_path, "app_home") == 0)
                    rel = ps3_path + length;
                else if (rel[0] == '/') rel++;
            }
        }
    ...
    struct stat st;
    if (!usrdir_layout && stat(host_path, &st) != 0) {
        const char* usr = strstr(ps3_path, "/USRDIR/");
        if (usr) {
            char alt[1024];
