...
#include <limits.h>               /* INT_MAX: unlimited q=1 unblock budget */
...
int64_t sys_event_queue_receive(ppu_context* ctx)
{
    ...
    /* Several permanently polling guest workers call this hundreds of times a
     * second when their SPU counterpart is absent.  Formatting every poll not
     * only floods the log; stderr's shared lock stalls unrelated frame-thread
     * diagnostics. Keep the full trace available explicitly. */
    static int wait_trace = -1;
    if (wait_trace < 0) wait_trace = getenv("PPU_WAIT_TRACE") ? 1 : 0;
    if (wait_trace)
    ...
    /* Taiko: satisfy the audio mixer's SPU round-trip.
     *
     * bnusAudioMixerLoop (guest func_0037958C, body of bnusCoreUpdateThread)
     * sends a 'STAT' request to the SPU audio worker via sys_event_port_send,
     * then spins:  do { r = receive(reply_q); } while (r7 != 'END ');
     *
     * The SPU images are never executed, so that reply never arrives and the
     * loop NEVER exits -- one core spins forever, the log fills with the q=5
     * receive flood (107k lines in one boot), and no audio block is ever mixed,
     * because the 0x2000-byte copy sits after the loop.
     *
     * Synthesize the completion so the mixer finishes an iteration: silence, but
     * forward progress instead of a permanent spin. Scoped by the caller's guest
     * return address, so ONLY that wait is affected and every other event queue
     * keeps real semantics.
     *
     * OPT-IN (TAIKO_AUDIO_END=1) -- NOT yet working, kept for the next attempt.
     * Measured 2026-08-11, three receive call sites exist in that loop:
     *     lr 0x003795C8 (q=2), 0x0037963C (q=5)  -- outer, retry on ETIMEDOUT
     *     lr 0x003796F8 (q=2)                    -- believed to be the 'END ' wait
     * Satisfying ALL of them (range 0x0037958C..0x00379748) does let the mixer
     * advance -- 289 file opens, boot alive -- but the loop then free-runs at
     * unbounded rate because the outer receives stop blocking, so the q=5 log
     * flood continues and nothing paces the mixer. Restricting to 0x00379680.. so
     * only 0x003796F8 is covered made the shim never fire at all, i.e. the inner
     * wait is not reached because the outer receive never succeeds first.
     *
     * So the real dependency is upstream: the outer receive needs a genuine audio
     * event (the SPU worker or a timer-driven substitute) before the 'END '
     * handshake is even reached. Synthesizing the reply alone is not enough.
     *
     * ponytail: this fakes the SPU's answer, it decodes nothing. Real audio needs
     *           the SPU worker (or ATRAC3+ behind cellAtrac); delete this shim
     *           then rather than keeping both. */
    {
        static int disabled = -1;
        uint32_t lr_ = (uint32_t)ctx->lr;
        if (disabled < 0) disabled = getenv("TAIKO_AUDIO_END") ? 0 : 1;   /* opt-in */
        if (!disabled && lr_ >= 0x00379680u && lr_ < 0x00379748u) {
            static int logged = 0;
            if (logged++ < 4)
                fprintf(stderr, "[audio] synthesizing 'END ' for the SPU mix round-trip "
                                "(q=%u lr=0x%08X) -- SPU audio worker is not executed\n",
                        queue_id, lr_);
            ctx->gpr[4] = 0; ctx->gpr[5] = 0; ctx->gpr[6] = 0;
            ctx->gpr[7] = 0x454E4420u;   /* 'END ' */
            if (event_addr != 0) {
                uint64_t* out = (uint64_t*)vm_to_host(event_addr);
                out[0] = 0; out[1] = 0; out[2] = 0;
                out[3] = bswap64((uint64_t)0x454E4420u);
            }
            return CELL_OK;
        }
    }
    ...
    if (getenv("YDKJ_HLE_DRAW") && queue_id == 1 && timeout_us == 0 && q->count == 0) {
        /* The 256 budget was enough for the title this was written for, but
         * Taiko's asset load needs far more: it drives AsyncFileLoaderWorkerLoop
         * through ~1000 files, and when the budget runs out q=1 blocks forever
         * again, the loader starves on its work semaphore, and loading stalls
         * mid-list. YDKJ_HLE_DRAW_MAX overrides it; 0 = unlimited. */
        static int s_cap = -1;
        if (s_cap < 0) {
            const char* c = getenv("YDKJ_HLE_DRAW_MAX");
            s_cap = c ? atoi(c) : 256;
            if (s_cap <= 0) s_cap = INT_MAX;
        }
        static int s_n1 = 0;
        if (s_n1 < s_cap) { s_n1++;
            if (event_addr != 0) {
...
int64_t sys_event_port_send(ppu_context* ctx)
{
    static int trace = -1;
    if (trace < 0) {
        const char* e = getenv("SYS_EVENT_TRACE");
        trace = (e && e[0] != '0') ? 1 : 0;
    }
    uint32_t port_id = LV2_ARG_U32(ctx, 0);
    ...
    if (trace)
        fprintf(stderr, "[evt] port_send(port=%u data=0x%llX/0x%llX/0x%llX)\n",
                port_id, (unsigned long long)data1, (unsigned long long)data2,
