...
typedef struct {
    ...
    uint32_t image_ea;       /* sys_spu_image captured at initialize */
    ...
    uint32_t args_ea;
    uint32_t args_size;
    uint64_t args[4];        /* sys_spu_thread_argument values (arg1..arg4) */
    ...
    uint8_t*            local_store;
    spu_context*        exec_ctx;   /* live raw lifted thread, if any */
    ...
    uint32_t            spup_queues[64];
    /* sys_spu_thread_bind_queue mappings used by stop 0x110
     * (sys_spu_thread_receive_event). */
    uint32_t            spuq_names[32];
    uint32_t            spuq_queues[32];
} spu_thread_t;
...
static uint64_t vm_read_be64(uint32_t guest_addr)
{
    uint64_t hi = vm_read_be32(guest_addr);
    uint64_t lo = vm_read_be32(guest_addr + 4);
    return (hi << 32) | lo;
}

static uint8_t* spu_thread_get_or_alloc_ls(spu_thread_t* t);

/* Materialize a parsed sys_spu_image in the thread's local store. The image
 * and argument structs are commonly stack temporaries, so this must happen at
 * sys_spu_thread_initialize rather than later on the asynchronous host thread. */
static void spu_thread_load_image(spu_thread_t* t, uint32_t img_ea)
{
    if (!t || !img_ea || !vm_base) return;
    uint32_t segs = vm_read_be32(img_ea + 8);
    uint32_t nsegs = vm_read_be32(img_ea + 12);
    if (!segs || !nsegs || nsegs > 32) return;
    ...
    for (uint32_t s = 0; s < nsegs; s++) {
        ...
        uint32_t dst  = vm_read_be32(b + 0x04);
        uint32_t size = vm_read_be32(b + 0x08);
        uint64_t src  = vm_read_be64(b + 0x10);
        if (dst > SPU_LS_SIZE || size > SPU_LS_SIZE - dst) {
            fprintf(stderr, "[SPU] image segment %u outside LS: dst=0x%X size=0x%X\n",
                    s, dst, size);
            continue;
        }
        if (type == 1 && src <= UINT32_MAX) {
            memcpy(ls + dst, vm_base + (uint32_t)src, size);
        } else if (type == 2) {
            memset(ls + dst, (uint8_t)src, size);
        }
    }
}
...
static int64_t sys_spu_thread_initialize_handler(ppu_context* ctx)
{
    ...
    t->args_ea   = args_ea;
    t->args_size = args_ea ? 32 : 0;
    if (args_ea) {
        for (int i = 0; i < 4; i++) t->args[i] = vm_read_be64(args_ea + i * 8);
    }
    spu_thread_load_image(t, img_ea);
    ...
    if (getenv("YDKJ_SPUIMG") && img_ea) {
        ...
        fprintf(stderr, "[SPUIMG]  args=%016llX %016llX %016llX %016llX\n",
                (unsigned long long)t->args[0], (unsigned long long)t->args[1],
                (unsigned long long)t->args[2], (unsigned long long)t->args[3]);
        fflush(stderr);
    }
...
static int64_t sys_spu_thread_set_argument_handler(ppu_context* ctx)
{
    ...
    if (t) {
        t->args_ea = arg_ea;
        t->args_size = arg_ea ? 32 : 0;
        if (arg_ea) for (int i = 0; i < 4; i++)
            t->args[i] = vm_read_be64(arg_ea + i * 8);
    }
...
static int64_t sys_spu_thread_connect_event_handler(ppu_context* ctx)
{
    ...
    spu_thread_t* t = spu_find_thread(tid);
    if (t) {
        t->connected_queue = eq;
        t->connect_spup = spup;
        if (spup < 64) t->spup_queues[spup] = eq;
    }
    fprintf(stderr, "[SPU] thread_connect_event tid=0x%X queue=0x%X et=0x%X spup=%u%s\n",
            tid, eq, et, spup, t ? "" : " (thread not found)");
    fflush(stderr);
    ctx->gpr[3] = 0;
    return 0;
}
                                      ...
                                      int is_intr, uint32_t value)
{
    ...
    if (!t || !t->exec_ctx || !is_intr) return;

    /* Exact sys_spu_thread_send_event ABI: WrOutIntrMbox contains
     * (spup << 24) | data0, the preceding ordinary outbound mailbox contains
     * data1, and sys_spu_thread_connect_event selects a queue per SPU port. */
    uint32_t spup = value >> 24;
    uint32_t data = spu_channel_read(&t->exec_ctx->ch_out_mbox);
    uint32_t q = spup < 64 ? t->spup_queues[spup] : 0;
    int rc = q ? sys_event_queue_push_by_id(q,
        0xFFFFFFFF53505501ULL, (uint64_t)spu_id,
        ((uint64_t)spup << 32) | (value & 0x00FFFFFFu), data) : -1;

    /* The synchronous SPU syscall returns the queue-send result through the
     * inbound mailbox. */
    spu_channel_write(&t->exec_ctx->ch_in_mbox, rc ? 0x8001000Au : 0);
    { static int s_w = 0; if (s_w++ < 64)
        fprintf(stderr, "[SPU->PPU] send_event spu=0x%X spup=%u data0=0x%06X data1=0x%08X -> q=%u rc=%d\n",
                spu_id, spup, value & 0x00FFFFFFu, data, q, rc); }
}

/* Service the raw-SPU stop 0x110 syscall. The SPU puts its logical queue name
 * in the outbound mailbox, then blocks. LV2 removes an event from the queue
 * previously attached by sys_spu_thread_bind_queue and returns four inbound
 * mailbox words: rc, data1, data2, data3. */
static int ydkj_spu_stop_deliver(spu_context* s, uint32_t stop_code)
{
    if (!s || stop_code != 0x110u) return 0;
    spu_thread_t* t = spu_find_thread(s->spu_id);
    if (!t || !spu_channel_has_data(&s->ch_out_mbox)) return 0;

    uint32_t spuq_name = spu_channel_read(&s->ch_out_mbox);
    uint32_t queue_id = 0;
    for (int i = 0; i < 32; i++) {
        if (t->spuq_queues[i] && t->spuq_names[i] == spuq_name) {
            queue_id = t->spuq_queues[i];
            break;
        }
    }

    sys_event_t event = {0};
    int rc = queue_id ? sys_event_queue_receive_by_id(queue_id, &event) : -1;
    if (s->ch_in_mbox.count != 0) {
        fprintf(stderr, "[SPU] receive_event tid=0x%X spuq=0x%08X: inbound mailbox busy (%u)\n",
                s->spu_id, spuq_name, s->ch_in_mbox.count);
        return 0;
    }
    spu_channel_push(&s->ch_in_mbox, rc ? 0x80010005u : 0u);
    if (!rc) {
        spu_channel_push(&s->ch_in_mbox, (uint32_t)event.data1);
        spu_channel_push(&s->ch_in_mbox, (uint32_t)event.data2);
        spu_channel_push(&s->ch_in_mbox, (uint32_t)event.data3);
    }
    { static int s_w = 0; if (s_w++ < 32)
        fprintf(stderr, "[SPU] receive_event tid=0x%X spuq=0x%08X q=%u rc=%d d1=%08X d2=%08X d3=%08X\n",
                s->spu_id, spuq_name, queue_id, rc, (uint32_t)event.data1,
                (uint32_t)event.data2, (uint32_t)event.data3); }
    return 1;
}

/* sys_spu_thread_bind_queue(id, queue_id, spuq_name) */
static int64_t sys_spu_thread_bind_queue_handler(ppu_context* ctx)
{
    uint32_t tid = (uint32_t)ctx->gpr[3];
    uint32_t queue_id = (uint32_t)ctx->gpr[4];
    uint32_t spuq_name = (uint32_t)ctx->gpr[5];
    spu_thread_t* t = spu_find_thread(tid);
    if (!t || queue_id == 0 || queue_id > SYS_EVENT_QUEUE_MAX ||
        !g_sys_event_queues[queue_id - 1].active) {
        ctx->gpr[3] = (uint64_t)(int64_t)(int32_t)CELL_ESRCH;
        return CELL_ESRCH;
    }
    if (g_sys_event_queues[queue_id - 1].type != SYS_SPU_QUEUE) {
    ...
    int free_slot = -1;
    for (int i = 0; i < 32; i++) {
        if (!t->spuq_queues[i] && free_slot < 0) free_slot = i;
        if (t->spuq_queues[i] &&
            (t->spuq_names[i] == spuq_name || t->spuq_queues[i] == queue_id)) {
    ...
    t->spuq_names[free_slot] = spuq_name;
    t->spuq_queues[free_slot] = queue_id;
    fprintf(stderr, "[SPU] thread_bind_queue tid=0x%X q=%u spuq=0x%08X\n",
            tid, queue_id, spuq_name);
    ctx->gpr[3] = 0;
    return 0;
}

/* sys_spu_thread_unbind_queue(id, spuq_name) */
static int64_t sys_spu_thread_unbind_queue_handler(ppu_context* ctx)
{
    uint32_t tid = (uint32_t)ctx->gpr[3];
    uint32_t spuq_name = (uint32_t)ctx->gpr[4];
    spu_thread_t* t = spu_find_thread(tid);
    if (!t) {
        ctx->gpr[3] = (uint64_t)(int64_t)(int32_t)CELL_ESRCH;
        return CELL_ESRCH;
...
/* Execute a raw lifted SPU image with the ABI values captured by
 * sys_spu_thread_initialize. Unlike SPURS one-shot workloads, a raw thread has
 * persistent mailbox/signal channels and receives arg1..arg4 directly in
 * registers r3..r6 rather than an EA pointing at the argument struct. */
int32_t spu_thread_run_lifted_raw(uint32_t tid, void (*entry)(spu_context*),
                                  int image_id)
{
    spu_thread_t* t = spu_find_thread(tid);
    if (!t || !entry) return -1;

    spu_context* s = (spu_context*)calloc(1, sizeof(*s));
    if (!s) return -1;
    spu_context_init(s, tid);
    s->spu_group_id = t->group_id;
    s->image_id = image_id;
    s->status = SPU_STATUS_RUNNING;
    s->channel_blocking = 1;
    if (t->local_store) memcpy(s->ls, t->local_store, SPU_LS_SIZE);
    /* sys_spu_thread_argument values are 64-bit SPU ABI scalars.  The SPU
     * register's preferred slot is big-endian word order: word 0 is the high
     * half and word 1 is the low half.  Assigning the host u64 directly put
     * the low word in word 0 on x86.  Code such as bnusCore's
     * shlqbyi(arg3, 4), used to extract a 32-bit EA from a u64 argument, then
     * produced zero and made every mixer DMA target address 0. */
    for (int i = 0; i < 4; i++) {
        s->gpr[3 + i]._u32[0] = (uint32_t)(t->args[i] >> 32);
        s->gpr[3 + i]._u32[1] = (uint32_t)t->args[i];
    }
    t->exec_ctx = s;

    fprintf(stderr,
            "[SPU] raw lifted start tid=0x%X image=%d args=%016llX/%016llX/%016llX/%016llX\n",
            tid, image_id, (unsigned long long)t->args[0],
            (unsigned long long)t->args[1], (unsigned long long)t->args[2],
            (unsigned long long)t->args[3]);
    fflush(stderr);
    { extern int spu_run_with_halt(void (*)(spu_context*), spu_context*);
      spu_run_with_halt(entry, s); }

    if (t->local_store) memcpy(t->local_store, s->ls, SPU_LS_SIZE);
    t->exec_ctx = NULL;
    fprintf(stderr, "[SPU] raw lifted stop tid=0x%X status=0x%X code=0x%X\n",
            tid, s->status, s->stop_code);
    fflush(stderr);
    free(s);
    return 0;
}
...
{
    uint32_t tid = (uint32_t)ctx->gpr[3];
    uint32_t value = (uint32_t)ctx->gpr[4];
    spu_thread_t* t = spu_find_thread(tid);
    if (!t || !t->exec_ctx) {
        ctx->gpr[3] = (uint64_t)(int64_t)(int32_t)0x80010005;
        return -1;
    }
    spu_channel_write(&t->exec_ctx->ch_in_mbox, value);
    if (getenv("YDKJ_MBOXTRACE")) {
        fprintf(stderr, "[SPU] write inbound mbox tid=0x%X value=0x%08X\n",
                tid, value);
        fflush(stderr);
    }
    ctx->gpr[3] = 0;
...
{
    uint32_t tid = (uint32_t)ctx->gpr[3];
    uint32_t number = (uint32_t)ctx->gpr[4];
    uint32_t value = (uint32_t)ctx->gpr[5];
    spu_thread_t* t = spu_find_thread(tid);
    if (!t || !t->exec_ctx || number > 1) {
        ctx->gpr[3] = (uint64_t)(int64_t)(int32_t)0x80010005;
        return -1;
    }
    spu_channel_write(&t->exec_ctx->ch_sig_notify[number], value);
    ctx->gpr[3] = 0;
    return 0;
}
...
void lv2_register_all_syscalls(lv2_syscall_table* tbl)
{
    ...
    { extern int (*g_spu_stop_hook)(spu_context*,uint32_t);
      g_spu_stop_hook = ydkj_spu_stop_deliver; }
