...
#include "../memory/vm.h"
#include <string.h>
#include <stdlib.h>   /* atoi: FLOW_STUCKSEM threshold */
...
int64_t sys_semaphore_wait(ppu_context* ctx)
{
    ...
    { static unsigned wait_trace_count;
      /* The NU::System::VSync worker waits on semaphore 1 every frame. Keep
       * that expected 60 Hz traffic from hiding one-shot startup waits. */
      if ((uint32_t)ctx->cia != 0x01019388u && wait_trace_count++ < 96)
          fprintf(stderr, "[WAIT] semaphore_wait(sem=%u timeout=%llu cia=0x%08X lr=0x%08X)\n",
    ...
    /* FLOW_STUCKSEM: name the wait a hang is sitting in. The entry trace above
     * is capped and fires before blocking, so it never shows which semaphore is
     * still outstanding once the title freezes. Report any wait that actually
     * blocks longer than the threshold (default 2000 ms), repeating every
     * threshold so a permanently stuck wait keeps announcing itself. */
    static int s_stuck_ms = -1;
    if (s_stuck_ms < 0) {
        const char* e = getenv("FLOW_STUCKSEM");
        s_stuck_ms = e ? (atoi(e) ? atoi(e) : 2000) : 0;
    }
#ifdef _WIN32
    uint64_t stuck_start = s_stuck_ms > 0 ? (uint64_t)GetTickCount64() : 0;
    DWORD result;
    if (s_stuck_ms > 0 && timeout_us == 0) {
        /* Infinite wait: poll so the report can fire while still blocked. */
        for (unsigned round = 1;; ++round) {
            result = WaitForSingleObject(s->sem_handle, (DWORD)s_stuck_ms);
            if (result != WAIT_TIMEOUT) break;
            fprintf(stderr,
                    "[STUCKSEM] sem=%u blocked %llums (infinite) cia=0x%08X lr=0x%08X tid=%lu\n",
                    sem_id,
                    (unsigned long long)((uint64_t)GetTickCount64() - stuck_start),
                    (uint32_t)ctx->cia, (uint32_t)ctx->lr,
                    (unsigned long)GetCurrentThreadId());
            (void)round;
        }
    } else if (timeout_us > 0 && timeout_us < 1000) {
