...
{
    const uint8_t* p = (const uint8_t*)vm_to_host(addr);
...
int64_t sys_cond_wait(ppu_context* ctx)
{
    ...
    /* Cond 5 is NU::Draw's sequence gate.  Log its object and sequence value
     * at a low rate so a stalled producer can be distinguished from a lost
     * host wake without flooding stderr (which can itself perturb timing). */
    if (cond_id == 5) {
        static volatile long s_draw_wait_count = 0;
        long n = __atomic_add_fetch(&s_draw_wait_count, 1, __ATOMIC_RELAXED);
        if (n <= 64 || (n & 0xFF) == 0) {
            uint32_t obj = (uint32_t)ctx->gpr[31];
            uint64_t seq = 0;
            if (obj != 0 && obj < 0x50000000u)
                seq = read_be64(obj + 0x10);
            fprintf(stderr,
                    "[DRAW-WAIT %ld] host=%lu guest=%llu lr=0x%08X obj=0x%08X want=%llu seq=%llu\n",
                    n, (unsigned long)ps3_host_thread_id(),
                    (unsigned long long)ctx->thread_id, (uint32_t)ctx->lr, obj,
...
int64_t sys_cond_signal(ppu_context* ctx)
{
    uint32_t cond_id = LV2_ARG_U32(ctx, 0);
    if (cond_id == 5) {
        static volatile long s_draw_signal_count = 0;
        long n = __atomic_add_fetch(&s_draw_signal_count, 1, __ATOMIC_RELAXED);
        if (n <= 64 || (n & 0xFF) == 0) {
            ...
            if (obj != 0 && obj < 0x50000000u)
                seq = read_be64(obj + 0x10);
            fprintf(stderr,
                    "[DRAW-SIGNAL %ld] host=%lu guest=%llu lr=0x%08X obj=0x%08X set=%llu seq=%llu\n",
                    n, (unsigned long)ps3_host_thread_id(),
                    (unsigned long long)ctx->thread_id, (uint32_t)ctx->lr, obj,
                    (unsigned long long)ctx->gpr[28],
                    (unsigned long long)seq);
        }
    }
...
#ifdef _WIN32
    /* Taiko's NU::Draw request manager uses condition 5 for both sides of the
     * queue: the worker waits for new work and the main thread waits for draw
     * completion.  WakeConditionVariable may select the waiter whose predicate
     * is still false; it goes back to sleep and the intended waiter then has no
     * future signal, freezing frame submission.  A broadcast is equivalent to
     * allowing a spurious wake and lets both roles re-check under the mutex. */
    if (cond_id == 5) WakeAllConditionVariable(&c->cv);
    else              WakeConditionVariable(&c->cv);
#else
    if (cond_id == 5) pthread_cond_broadcast(&c->cv);
    else              pthread_cond_signal(&c->cv);
#endif
