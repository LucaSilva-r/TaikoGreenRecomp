...
static struct { uint32_t nid; const char* name; hle_ctx_fn fn; } g_ctx[HLE_CTX_CAP];
...
extern "C" void ps3_hle_register_ctx(uint32_t nid, const char* name, hle_ctx_fn fn)
{
    ...
    if (g_ctx_count < HLE_CTX_CAP) {
        g_ctx[g_ctx_count].nid = nid;
        g_ctx[g_ctx_count].name = name;
        g_ctx[g_ctx_count].fn = fn;
        g_ctx_count++;
        if (nid == 0x23654375u || nid == 0xBDF21B0Fu)
            fprintf(stderr, "[taiko_sail] registered %s NID=0x%08X slot=%u\n",
                    name ? name : "?", nid, g_ctx_count - 1);
    }
}
...
static HleProfileEntry g_hle_profile[1024];
static std::chrono::steady_clock::time_point g_hle_profile_report_at;

static const char* hle_profile_name(uint32_t nid)
{
    for (uint32_t i = 0; i < g_ctx_count; ++i)
        if (g_ctx[i].nid == nid) return g_ctx[i].name ? g_ctx[i].name : "ctx";
    ps3_nid_entry* e = g_hle_inited ? ps3_nid_table_find(&g_hle_nids, nid) : nullptr;
    return e && e->name ? e->name : "?";
}

static void hle_profile_record(uint32_t nid, uint32_t elapsed_us)
{
    uint32_t slot = (nid ^ (nid >> 16)) & 1023u;
    HleProfileEntry* entry = nullptr;
    for (uint32_t n = 0; n < 1024; ++n) {
        HleProfileEntry& candidate = g_hle_profile[(slot + n) & 1023u];
        if (candidate.nid == nid || candidate.nid == 0) {
            if (candidate.nid == 0) candidate.nid = nid;
            entry = &candidate;
            break;
        }
    }
    ...
    if (g_hle_profile_report_at.time_since_epoch().count() == 0)
        g_hle_profile_report_at = now + std::chrono::seconds(5);
    if (now < g_hle_profile_report_at) return;

    fprintf(stderr, "[HLEPROF] top main-thread HLE time over 5s:");
    for (unsigned rank = 0; rank < 12; ++rank) {
        HleProfileEntry* best = nullptr;
        for (auto& candidate : g_hle_profile) {
            if (!candidate.calls || (best && candidate.total_us <= best->total_us)) continue;
            bool already = false;
            for (unsigned prior = 0; prior < rank; ++prior) {
                /* Entries selected on earlier ranks have their high bit marked. */
                if (candidate.calls & 0x80000000u) { already = true; break; }
            }
            if (!already) best = &candidate;
        }
        if (!best) break;
        fprintf(stderr, " %08X(%s)=%u/%lluus max=%uus",
                best->nid, hle_profile_name(best->nid), best->calls,
                (unsigned long long)best->total_us, best->max_us);
        best->calls |= 0x80000000u;
    }
    fprintf(stderr, "\n");
    for (auto& candidate : g_hle_profile) candidate = HleProfileEntry{};
    g_hle_profile_report_at = now + std::chrono::seconds(5);
}

struct HleProfileGuard {
    uint32_t nid;
    bool active;
    std::chrono::steady_clock::time_point start;
    HleProfileGuard(uint32_t n, const ppu_context* ctx) : nid(n) {
        static const bool enabled = getenv("PPU_HLE_PROFILE") != nullptr;
        active = enabled && ctx && ctx->thread_id == 1;
    ...
    ~HleProfileGuard() {
        ...
        hle_profile_record(nid, us > 0xFFFFFFFFll ? 0xFFFFFFFFu : (uint32_t)us);
    }
};
...
{
    return (void*)ps3_host_image_base((const void*)&ps3_GetModuleHandleA);
}
#endif
...
extern "C" void ps3_hle_call(uint32_t nid, ppu_context* ctx)
{
    ...
    /* These three sysPrxForUser CRT calls account for >95% of Taiko's HLE
     * traffic during attract. They cannot be supplied by the loaded libsre PRX
     * and already have context-aware handlers in ppu_sysprx.cpp. Bypass the
     * generic resolver in normal runs, but retain it whenever its diagnostic
     * hooks were explicitly requested. _TocGuard still preserves the ELFv1
     * caller ABI on this early return. */
    static const bool hot_sysprx_enabled =
        !getenv("PPU_HLE_SLOWPATH") && !getenv("PS3_HLE_TRACE") &&
        !getenv("FLOW_HLETRACE") && !getenv("FLOW_BADLOCK") &&
        !getenv("FLOW_SPINLR") && !getenv("FLOW_HLETOC") &&
        !getenv("YDKJ_GFXSCAN");
    if (hot_sysprx_enabled && ppu_sysprx_try_hot_hle(nid, ctx)) {
        switch (nid) {
        case 0x1573DC3Fu: g_last_hle_name = "sys_lwmutex_lock"; break;
        case 0x1BC200F4u: g_last_hle_name = "sys_lwmutex_unlock"; break;
        case 0x350D454Eu: g_last_hle_name = "sys_ppu_thread_get_id"; break;
        }
        return;
    }
