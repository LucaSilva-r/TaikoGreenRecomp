...
#define GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS 0u
#define GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT 0u
static inline ULONGLONG GetTickCount64(void)
{
    return (ULONGLONG)ps3_host_monotonic_ms();
}
static inline unsigned short RtlCaptureStackBackTrace(
    unsigned long skip, unsigned long count, void** frames, unsigned long* hash)
{
    (void)skip; (void)hash;
    return (unsigned short)ps3_host_capture_backtrace(frames, count);
}
static inline void* GetModuleHandleA(const char*)
{
    return (void*)ps3_host_image_base((const void*)&GetModuleHandleA);
}
static inline int GetModuleHandleExA(unsigned long, LPCSTR address, HMODULE* out)
{
    *out = (void*)ps3_host_image_base((const void*)address);
    return *out != NULL;
}
#endif
...
/* Windows reserves the complete 4 GB guest window and commits 64 KB chunks on
 * first touch.  SPU DMA used to call VirtualQuery for every transfer outside
 * the fixed main-memory arena to distinguish a real guest allocation from a
 * garbage EA.  Active audio performs thousands of those transfers per second;
 * VirtualQuery is sufficiently expensive under Windows (and especially Wine)
 * to stall the real-time mixer.
 *
 * Keep the same safety check in a lock-free 4 KB accessibility bitmap.  The
 * demand-commit VEH and the explicit VM helpers update it only after the OS
 * operation succeeds, so DMA can reject untouched reservation pages without a
 * kernel call on its hot path. */
#ifdef _WIN32
#define VM_TRACK_PAGE_SHIFT 12u
#define VM_TRACK_PAGE_COUNT (1u << (32u - VM_TRACK_PAGE_SHIFT))
#define VM_TRACK_WORD_COUNT (VM_TRACK_PAGE_COUNT / 64u)
static volatile LONG64 s_vm_accessible_pages[VM_TRACK_WORD_COUNT];

extern "C" void vm_note_accessible_range(uint32_t addr, uint32_t size, int accessible)
{
    if (!size) return;
    uint64_t end64 = (uint64_t)addr + size - 1u;
    if (end64 >= 0x100000000ull) end64 = 0xffffffffull;
    uint32_t first = addr >> VM_TRACK_PAGE_SHIFT;
    uint32_t last = (uint32_t)end64 >> VM_TRACK_PAGE_SHIFT;
    for (uint32_t page = first; page <= last; ++page) {
        volatile LONG64* word = &s_vm_accessible_pages[page >> 6];
        LONG64 mask = (LONG64)(1ull << (page & 63));
        if (accessible)
            InterlockedOr64(word, mask);
        else
            InterlockedAnd64(word, ~mask);
    }
}

extern "C" int vm_range_is_accessible(uint32_t addr, uint32_t size)
{
    if (!size) return 0;
    uint64_t end64 = (uint64_t)addr + size - 1u;
    if (end64 >= 0x100000000ull) return 0;
    uint32_t first = addr >> VM_TRACK_PAGE_SHIFT;
    uint32_t last = (uint32_t)end64 >> VM_TRACK_PAGE_SHIFT;
    for (uint32_t page = first; page <= last; ++page) {
        volatile LONG64* word = &s_vm_accessible_pages[page >> 6];
        LONG64 bits = InterlockedCompareExchange64(word, 0, 0);
        if (!(bits & (LONG64)(1ull << (page & 63)))) return 0;
    }
    return 1;
}

extern "C" void vm_reset_accessible_ranges(void)
{
    memset((void*)s_vm_accessible_pages, 0, sizeof(s_vm_accessible_pages));
}
#endif
 ...
 *
 * The recompiled lwarx sets ctx->reserve_addr = ea; stwcx then compare-and-swaps
 * the reserved value. A bare value-CAS is ABA-VULNERABLE: for a lock-free
 * free-list pop (`lwarx head; next = *head; stwcx next, head`), a concurrent
 * pop+push that restores head to its old bits lets a stale stwcx succeed -- the
 * popped `next` then points at a reused block, and the guest heap is corrupt.
 * Real PPC loses the reservation on ANY store to the granule, so that stale
 * stwcx would FAIL. This is the mechanism the title's allocator and free lists
 * rely on, and emulating it is what turns nondeterministic heap corruption
 * (random hangs, aborts, 0xDDDDDDxx pointers) back into deterministic behaviour.
 *
 * Emulation: recheck + CAS + break run under one per-address lock, and the
 * caller's own reservation is re-validated inside that critical section (via
 * the thread-local g_active_ctx) -- otherwise a break can land between the
 * caller's `reserve_addr == ea` test and its CAS. On success every OTHER
 * thread's reservation on the word is clobbered to an impossible value so its
 * own guard fails and it retries.
 *
 * Locks are sharded by the 16-byte block of `ea`: one global lock convoyed all
 * stwcx traffic (workers spinning on a descheduled holder). Same-structure
 * contention still serializes on one slot, which is inherent.
 *
 * PPU_RESV_STORE=1 additionally breaks reservations on PLAIN stores
 * (vm_write32/64), matching real PPC exactly. Default off: it costs a scan on
 * the hottest path in the runtime, and stwcx-vs-stwcx covers the lock-free
 * cases the guest actually uses.
 * -----------------------------------------------------------------------*/
extern "C" thread_local ppu_context* g_active_ctx;   /* fwd decl (defined below) */
...
static int           g_resv_ctx_n = 0;
static volatile int  g_resv_locks[PPU_RESV_LOCKS];
extern "C" int       g_resv_store_active = -1;   /* -1 uninit, 0 off, 1 on */

extern "C" void ppu_resv_register(ppu_context* c)
{
    if (g_resv_store_active < 0)
        g_resv_store_active = getenv("PPU_RESV_STORE") ? 1 : 0;
    int i = __atomic_fetch_add(&g_resv_ctx_n, 1, __ATOMIC_ACQ_REL);
    if (i < PPU_RESV_MAX) g_resv_ctxs[i] = c;
}

static inline int resv_n(void)
{
    int n = __atomic_load_n(&g_resv_ctx_n, __ATOMIC_ACQUIRE);
    return n > PPU_RESV_MAX ? PPU_RESV_MAX : n;
}

/* Break every reservation on ea's word. Caller holds the slot lock. */
static inline void ppu_resv_break(uint64_t addr)
{
    uint64_t a = (uint32_t)addr;   /* reserve_addr holds the (uint32_t) ea */
    int n = resv_n();
    for (int i = 0; i < n; i++) {
...
/* Plain-store break (PPU_RESV_STORE). Best-effort/unlocked: the break is a
 * single word write, and the value-CAS backstops the narrow race. */
extern "C" void ppu_resv_break_store(uint64_t ea)
{
    ...
    for (int i = 0; i < n; i++) {
        ppu_context* c = g_resv_ctxs[i];
        if (c && c->reserve_addr < PPU_RESV_INVALID &&
            ((uint32_t)c->reserve_addr & ~3u) == a)
            c->reserve_addr = PPU_RESV_INVALID;
    }
}

static inline volatile int* resv_slot(uint64_t ea)
{
    return &g_resv_locks[((uint32_t)ea >> 4) & (PPU_RESV_LOCKS - 1)];
}
static inline void resv_lock(volatile int* L)
{
...
/* PPU_RESV_OFF=1 -> plain value-CAS (pre-fix baseline, for A/B diagnosis). */
static inline int resv_off(void) { static int v = -1; if (v < 0) v = getenv("PPU_RESV_OFF") ? 1 : 0; return v; }

/* RESV_DIAG: a stwcx from a ctx that never registered is a thread whose
 * reservations nobody breaks -- ABA slips through for it. One line per ctx. */
static inline int resv_diag(void) { static int v = -1; if (v < 0) v = getenv("RESV_DIAG") ? 1 : 0; return v; }
static void resv_check_reg(ppu_context* self)
{
    int n = resv_n();
    for (int i = 0; i < n; i++) if (g_resv_ctxs[i] == self) return;
    static ppu_context* seen[64]; static int sn = 0;
    for (int i = 0; i < sn; i++) if (seen[i] == self) return;
    if (sn < 64) seen[sn++] = self;
    fprintf(stderr, "[RESV-UNREG] stwcx/stdcx by UNREGISTERED ctx=%p tid=%llu (n_reg=%d)\n",
            (void*)self, (unsigned long long)self->thread_id, g_resv_ctx_n);
    fflush(stderr);
}

/* `expected` / `val` are host-endian; guest memory is big-endian. */
extern "C" int ppu_stwcx32(uint64_t ea, uint32_t expected, uint32_t val)
{
    ...
    uint32_t* p = (uint32_t*)(vm_base + ea);
    if (resv_off())
        return __atomic_compare_exchange_n(p, &exp_raw, new_raw, 0,
                                           __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST) ? 1 : 0;
    ppu_context* self = g_active_ctx;
    if (resv_diag() && self) resv_check_reg(self);
    volatile int* L = resv_slot(ea);
    resv_lock(L);
    if (self && self->reserve_addr != (uint32_t)ea) { resv_unlock(L); return 0; }
    int ok = __atomic_compare_exchange_n(p, &exp_raw, new_raw, 0,
                                         __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST) ? 1 : 0;
...
extern "C" int ppu_stdcx64(uint64_t ea, uint64_t expected, uint64_t val)
{
    ...
    uint64_t* p = (uint64_t*)(vm_base + ea);
    if (resv_off())
        return __atomic_compare_exchange_n(p, &exp_raw, new_raw, 0,
                                           __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST) ? 1 : 0;
    ppu_context* self = g_active_ctx;
    if (resv_diag() && self) resv_check_reg(self);
    volatile int* L = resv_slot(ea);   /* ea and ea+4 share a 16-byte-block slot */
    resv_lock(L);
    if (self && self->reserve_addr != (uint32_t)ea) { resv_unlock(L); return 0; }
    int ok = __atomic_compare_exchange_n(p, &exp_raw, new_raw, 0,
                                         __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST) ? 1 : 0;
    if (ok) { ppu_resv_break(ea); ppu_resv_break(ea + 4); }  /* store spans two words */
    resv_unlock(L);
    return ok;
}
 ...
 *
 * Every PPU thread needs its OWN block: r13 is how the guest reaches
 * thread-local state, and this title keeps a per-thread MATRIX STACK there --
 * the scene-graph push/pop of v28-v31 goes through a descriptor at r13-0x6FEC
 * ({top, base, count, capacity}).  Handing every thread the same block makes
 * concurrent traversals share one stack cursor and trample each other's
 * current transform.
 *
 * One allocator for both callers (this file's thread entry and sysprx's
 * sys_initialize_tls) so their bump pointers cannot overlap. */
static uint32_t   s_tls_next = 0x0E000000u;
static std::mutex s_tls_lock;

extern "C" uint32_t ppu_tls_alloc(uint32_t seg_addr, uint32_t seg_size, uint32_t mem_size)
{
    if (!seg_addr && !mem_size) {          /* caller has no template of its own */
    ...
    uint32_t block;
    {
        std::lock_guard<std::mutex> guard(s_tls_lock);
        block = s_tls_next;
        s_tls_next += ((mem_size + 0x7000u + 0x1000u) + 0xFFFu) & ~0xFFFu;
    }
    ...
    return block + 0x7000u;                /* thread pointer; TLS data at r13-0x7000 */
}

/* Immutable copy of the ELF's function-descriptor table. Some titles reuse or
 * accidentally clobber live .opd storage during startup. Direct calls are
 * unaffected, but callbacks and sys_ppu_thread_create receive an OPD address
 * and otherwise resolve it to zero. The original table is a safe fallback only
 * when the live descriptor no longer names registered code. */
struct PpuOpdSnapshot { uint32_t code, toc; };
static PpuOpdSnapshot* s_opd_snapshot = NULL;
static uint32_t s_opd_snapshot_base = 0;
static uint32_t s_opd_snapshot_count = 0;

static int ppu_opd_snapshot_resolve(uint32_t opd, uint32_t* code, uint32_t* toc)
{
    if (!s_opd_snapshot || opd < s_opd_snapshot_base || ((opd - s_opd_snapshot_base) & 7u))
        return 0;
    uint32_t index = (opd - s_opd_snapshot_base) / 8u;
    if (index >= s_opd_snapshot_count || !s_opd_snapshot[index].code)
        return 0;
    if (code) *code = s_opd_snapshot[index].code;
    if (toc)  *toc  = s_opd_snapshot[index].toc;
    return 1;
}
...
/* The memory helpers are the hottest functions in lifted PPU execution. Their
 * diagnostics used to run several function-local thread_local checks on every
 * guest load/store even when no diagnostic environment variable was enabled.
 * MinGW implements those accesses through pthread_getspecific(), making a
 * nominal four-byte guest load surprisingly expensive. Cache one process-wide
 * decision before guest threads start and take a genuinely minimal fast path
 * during normal play. */
...
static void ppu_vm_diag_init(void)
{
    static const char* const vars[] = {
        "PPU_VM_DIAG", "YDKJ_HOTMAP", "YDKJ_LV2_SAT",
    ...
    if (g_vm_diag_enabled)
        fprintf(stderr, "[vm] guest-memory diagnostics enabled (slow path)\n");
}
...
extern "C" thread_local ppu_context* g_active_ctx;  /* defined below; needed here for spin backtrace */
extern "C" void ppu_dump_guest_stack(ppu_context* ctx, const char* tag); /* defined below */
...
extern "C" void ydkj_parse_yield(void) { ps3_host_sleep_ms(1); }
...
static thread_local uint32_t g_last_rd_addr = 0;
static thread_local uint32_t g_last_rd_val  = 0;
...
static thread_local uint32_t g_vcall_stk[128];
static thread_local int      g_vcall_sp = 0;
extern "C" {
uint8_t  vm_read8 (uint64_t a) { if (vm_oob((uint32_t)a,1)) return 0;
    if (!g_vm_diag_enabled) return vm_base[(uint32_t)a];
...
#endif
    { static thread_local uint32_t last=0xFFFFFFFFu; static thread_local uint32_t n=0;
      if ((uint32_t)a==last) { if (++n==200000) {
        /* Rate report: this fires every 200k consecutive reads of one address.
         * Taiko's heap debug-fill re-reads its 0xCD pattern byte once per byte
         * written, so a long "spin" here is usually a multi-megabyte memset
         * making slow progress rather than a deadlock. Timing tells them
         * apart: steady growth = progress, frozen total = a real spin. */
        { static thread_local unsigned long long tot8 = 0;
          static thread_local unsigned long long t0 = 0;
          unsigned long long now = (unsigned long long)GetTickCount64();
          if (!t0) t0 = now;
          tot8 += 200000ull;
          fprintf(stderr, "[HOTREAD8] 0x%08X reads=%lluk elapsed=%llums (%llu KB/s if fill)\n",
                  (uint32_t)a, tot8/1000ull, now - t0,
                  (now > t0) ? (tot8 / (now - t0)) : 0ull); }
        n=0;
        /* PS3_SPINBT8=<addr>: dump the guest call chain the first time this byte
         * is spun on. vm_read32 already had this; a byte-wide poll (a bool/flag
         * rather than a label or FIFO get-pointer) had no way to name its
         * waiter, which is exactly what a stuck boot needs. */
        { static int64_t wa=-2; if(wa==-2){const char*e=getenv("PS3_SPINBT8"); wa=e?(int64_t)strtoul(e,0,0):-1;}
          if(wa>=0 && (uint32_t)a==(uint32_t)wa){ static int once=0; if(once++ < 6){
            if (g_active_ctx) { ppu_dump_guest_stack(g_active_ctx, "SPIN8");
              /* Full GPR set: a spin's cause is usually an argument register
               * that arrived wrong (here r31 = the allocator's alignment). */
              for (int gi=0; gi<32; gi+=8) { fprintf(stderr,"[SPIN8] r%-2d:",gi);
                for (int gj=0; gj<8; gj++) fprintf(stderr," %08X",(uint32_t)g_active_ctx->gpr[gi+gj]);
                fprintf(stderr,"\n"); } }
            /* Host backtrace too: the guest-stack scanner walks saved words and
             * happily reports whatever allocator ran last, and ctx->lr is stale
             * once the spin loop is a leaf. The host return chain IS the guest
             * call chain here -- each lifted guest function is a host function
             * -- so resolving these RVAs against the exe's symbols names the
             * spinning function exactly. */
            { void* bt[32]; unsigned short fr=RtlCaptureStackBackTrace(0,32,bt,0);
              char* mb=(char*)GetModuleHandleA(0);
              fprintf(stderr,"[SPIN8 0x%08X] host rva:",(uint32_t)a);
              for(int i=0;i<fr;i++) fprintf(stderr," %llX",(unsigned long long)((char*)bt[i]-mb));
              fprintf(stderr,"\n"); } } } } } }
      else { last=(uint32_t)a; n=0; } }
    return vm_base[(uint32_t)a]; }
uint16_t vm_read16(uint64_t a) { if (vm_oob((uint32_t)a,2)) return 0;
    if (!g_vm_diag_enabled) { uint16_t v; memcpy(&v, vm_base + (uint32_t)a, 2); return __builtin_bswap16(v); }
    vm_hotmap((uint32_t)a,2); uint16_t v; memcpy(&v, vm_base + (uint32_t)a, 2);
    { static thread_local uint32_t last=0xFFFFFFFFu; static thread_local uint32_t n=0;
      if ((uint32_t)a==last) { if (++n==200000) { fprintf(stderr, "[HOTREAD16] spinning on 0x%08X (=0x%04X)\n", (uint32_t)a, __builtin_bswap16(v)); n=0;
#ifdef _WIN32
        /* PS3_SPINBT16=<addr>: name the waiter on a halfword poll. read8 and
         * read32 already had this; a 16-bit flag had no way to identify its
         * spinner, which is exactly what a stuck loader needs. */
        { static int64_t wa=-2; if(wa==-2){const char*e=getenv("PS3_SPINBT16"); wa=e?(int64_t)strtoul(e,0,0):-1;}
          if(wa>=0 && (uint32_t)a==(uint32_t)wa){ static int once=0; if(once++ < 4){
            extern void ppu_dump_guest_stack(ppu_context*, const char*);
            if (g_active_ctx) ppu_dump_guest_stack(g_active_ctx, "SPIN16");
            void* bt[32]; unsigned short fr=RtlCaptureStackBackTrace(0,32,bt,0);
            char* mb=(char*)GetModuleHandleA(0);
            fprintf(stderr,"[SPIN16 0x%08X] guest cia=0x%08X lr=0x%08X host rva:",(uint32_t)a,
                    g_active_ctx?(uint32_t)g_active_ctx->cia:0, g_active_ctx?(uint32_t)g_active_ctx->lr:0);
            for(int i=0;i<fr;i++) fprintf(stderr," %llX",(unsigned long long)((char*)bt[i]-mb));
            fprintf(stderr,"\n"); fflush(stderr); } } }
#endif
      } } else { last=(uint32_t)a; n=0; } }
...
uint32_t vm_read32(uint64_t a) { if (vm_oob((uint32_t)a,4)) return 0;
    ...
    { static int en=-1; if(en<0) en=getenv("YDKJ_SPINCIA")?1:0;
      if(en && g_active_ctx){ static thread_local uint32_t rc=0;
        if(++rc >= 3000000u){ rc=0;
...
uint64_t vm_read64(uint64_t a) { if (vm_oob((uint32_t)a,8)) return 0;
    if (!g_vm_diag_enabled) { uint64_t v; memcpy(&v, vm_base + (uint32_t)a, 8); return __builtin_bswap64(v); }
    vm_hotmap((uint32_t)a,8); uint64_t v; memcpy(&v, vm_base + (uint32_t)a, 8);
#ifdef VM_SAMPLE_READS
...
void vm_write16(uint64_t a, uint16_t v) { if (vm_oob((uint32_t)a,2)) return;
    if (!g_vm_diag_enabled) { v = __builtin_bswap16(v); memcpy(vm_base + (uint32_t)a, &v, 2); return; }
...
void vm_write32(uint64_t a, uint32_t v) { if (vm_oob((uint32_t)a,4)) return;
    ...
    /* FLOW_WVAL_LO/HI bound the DESTINATION range, and FLOW_WVAL_N raises the hit
     * cap.  Without them a value that also appears in loaded file data (e.g. an
     * RSX vertex-format word read straight out of a model file) burns every slot
     * during loading, long before the store into the FIFO you actually want. */
    { static int64_t wv=-2; static uint32_t lo=0,hi=0; static int cap=6;
      if (wv==-2){ const char* e=getenv("FLOW_WVAL"); wv=e?(int64_t)strtoul(e,0,16):-1;
        const char* l=getenv("FLOW_WVAL_LO"); const char* h=getenv("FLOW_WVAL_HI");
        const char* n=getenv("FLOW_WVAL_N");
        lo=l?(uint32_t)strtoul(l,0,16):0; hi=h?(uint32_t)strtoul(h,0,16):0;
        if(n) cap=atoi(n); }
      if (wv>=0 && v==(uint32_t)wv && (uint32_t)a < 0x50000000u
          && (hi==0 || ((uint32_t)a>=lo && (uint32_t)a<hi))) { static int _n=0; if(_n++<cap){
...
/* Cross-fragment continuation state. Each 8-pointer row is one 64-byte cache
 * line on x64, so active guest threads neither race nor false-share the slot
 * checked after every lifted call. */
typedef void (*ppu_trampoline_fn)(void*);
extern "C" alignas(64) ppu_trampoline_fn g_trampoline_slots[256][8] = {};
alignas(64) static int g_dispatch_depth[256][16] = {};

static inline unsigned ppu_slot(const ppu_context* ctx)
{
    return (unsigned)ctx->thread_id & 255u;
}

static inline ppu_trampoline_fn& ppu_trampoline_for(ppu_context* ctx)
{
    return g_trampoline_slots[ppu_slot(ctx)][0];
}
...
extern "C" thread_local ppu_context* g_active_ctx = nullptr;
...
/* ponytail: fixed slots, raise if a title ever needs more than 8 hook files. */
static void (*g_project_register_hooks[8])(void);
static uint32_t g_project_register_hook_count = 0;

/* Let a title install narrowly scoped guest-function overrides after the
 * generated table has been registered.  The setter is safe to call from a
 * project constructor; ppu_recomp_register() runs later from the boot harness.
 * Several translation units may register independently, so the callbacks
...
extern "C" void ppu_set_project_register_hooks(void (*register_hooks)(void))
{
    ...
    if (g_project_register_hook_count >=
        sizeof(g_project_register_hooks) / sizeof(g_project_register_hooks[0])) {
        fprintf(stderr, "[ppu] project hook table full -- %p dropped\n",
                (void*)register_hooks);
        return;
    }
    g_project_register_hooks[g_project_register_hook_count++] = register_hooks;
}
...
extern "C" void ppu_recomp_register(void)
{
    ...
    for (uint32_t i = 0; i < g_project_register_hook_count; i++)
        g_project_register_hooks[i]();
}

/* ---------------------------------------------------------------------------
 * Guest-heap free ledger
 *
 * The title's allocator poisons freed blocks with 0xDD, so a use-after-free is
 * easy to SEE -- a pointer, vtable slot or TOC that reads back 0xDDDDDDxx --
 * but the poison says nothing about WHO freed the block, which is the only
 * question worth answering. The lifted guest free() (func_005A93CC) reports
 * every release here; the garbage-vcall guard then names the freeing call site
 * and thread for the object it just caught in the act.
 *
 * Ring buffer, no lock: a record lost to a race costs one diagnostic line,
 * whereas a mutex here would perturb the very race being hunted.
 * ------------------------------------------------------------------------*/
enum { PPU_FREE_RING = 8192 };
struct PpuFreeRec { uint32_t ptr, lr, tid, seq; };
static PpuFreeRec g_free_ring[PPU_FREE_RING];
static uint32_t   g_free_seq = 0;

extern "C" void ppu_note_free(uint32_t ptr, uint32_t lr, uint32_t tid)
{
    if (!ptr) return;            /* free(NULL) is legal and owns no block */
    uint32_t s = __atomic_fetch_add(&g_free_seq, 1u, __ATOMIC_RELAXED);
    PpuFreeRec* r = &g_free_ring[s % PPU_FREE_RING];
    r->ptr = ptr; r->lr = lr; r->tid = tid; r->seq = s + 1;
}

/* Most recent recorded free of a block that could contain `addr`. Blocks are
 * small, so a 64 KB back-scan covers any object seen so far. */
static const PpuFreeRec* ppu_lookup_free(uint32_t addr)
{
    const PpuFreeRec* best = nullptr;
    for (unsigned i = 0; i < PPU_FREE_RING; i++) {
        const PpuFreeRec* r = &g_free_ring[i];
        if (!r->seq || r->ptr > addr || addr - r->ptr >= 0x10000u) continue;
        if (!best || r->seq > best->seq) best = r;
    }
    return best;
}

/* Print the free record covering `addr`, if any. `what` labels the pointer. */
static void ppu_report_free_of(uint32_t addr, const char* what)
{
    /* A null/tiny pointer is an uninitialised field, not a dead block; asking
     * the ledger about it only produces noise. */
    if (addr < 0x10000u) return;
    const PpuFreeRec* r = ppu_lookup_free(addr);
    uint32_t now = __atomic_load_n(&g_free_seq, __ATOMIC_RELAXED);
    if (r)
        fprintf(stderr, "      [freelog] %s 0x%08X is +0x%X into a block freed by "
                        "lr=0x%08X tid=%u (%u frees ago)\n",
                what, addr, addr - r->ptr, r->lr, r->tid, now - r->seq);
    else
        fprintf(stderr, "      [freelog] %s 0x%08X: no recorded free covers it "
                        "(%u frees logged)\n", what, addr, now);
}
...
extern "C" void ppu_dump_guest_stack(ppu_context* ctx, const char* tag)
{
    ...
    uint32_t last = 0;
    /* Thread stacks have an inaccessible guard page immediately after the
     * preceding allocation.  A blind 700-word diagnostic scan can cross that
     * boundary when a thread is blocked near its stack top.  Stay within the
     * currently accessible page; this dumper must never turn a harmless wait
     * into a host fault. */
    const uint32_t guest_page_size = 0x1000u;
    uint32_t scan_end = (sp & ~(guest_page_size - 1u)) + guest_page_size;
    for (int i = 0; i < 700 && gp < 1100; i++) {
        uint32_t a = sp + i*4; if (a + 4u > scan_end || vm_oob(a,4)) break;
...
{
    /* g_active_ctx is installed once at each top-level guest entry. Rewriting
     * the same MinGW-emulated TLS pointer on every indirect call was pure cost. */
    ...
    /* FLOW_NOSPILL also gates THIS spill, not just the one in ps3_hle_call.
     * A tail-entry wrapper runs on its caller's frame, so r1 here can still be
     ...
     * three). r2 must always be one of them, so anything else is corruption,
     * and the FIRST violation is the one worth seeing: after that the wrong
     * base makes every TOC-relative load return garbage, and the failure
     * surfaces thousands of instructions later somewhere unrelated. */
    { static uint32_t toc_ok[8]; static int n_toc = -1;
      if (n_toc < 0) { n_toc = 0; const char* e = getenv("PS3_TOC_SET");
          for (; e && *e && n_toc < 8; ) { toc_ok[n_toc++] = (uint32_t)strtoul(e, (char**)&e, 0);
                                           while (*e == ',' || *e == ' ') e++; } }
      if (n_toc > 0) { uint32_t r2 = (uint32_t)ctx->gpr[2]; int ok = 0;
          for (int i = 0; i < n_toc; i++) if (r2 == toc_ok[i]) { ok = 1; break; }
          if (!ok) { static int _n = 0; if (_n++ < 6) {
              fprintf(stderr, "[TOCBAD] r2=0x%08X not a declared TOC; ctr=0x%08X lr=0x%08X\n",
                      r2, (uint32_t)ctx->ctr, (uint32_t)ctx->lr);
              /* A TOC of 0xDDDDDDxx came out of a poisoned (freed) block --
               * name the free that killed the object we are calling through. */
              ppu_report_free_of((uint32_t)ctx->gpr[3], "r3");
              /* A call target inside the guest heap is code read out of data:
               * look that up too, since it usually lands in the dead block. */
              if ((uint32_t)ctx->ctr >= 0x40000000u && (uint32_t)ctx->ctr < 0x50000000u)
                  ppu_report_free_of((uint32_t)ctx->ctr, "ctr");
              ppu_dump_guest_stack(ctx, "TOCBAD"); } } } }
#ifdef _WIN32
    ...
    if (fn) {
        ...
        int& s_depth = g_dispatch_depth[ppu_slot(ctx)][0];
        if (s_depth > 4000) {
            static int warned = 0;
            if (warned++ < 8)
        ...
        fn(ctx);
        while (ppu_trampoline_for(ctx)) {
            void (*tf)(void*) = ppu_trampoline_for(ctx);
            ppu_trampoline_for(ctx) = 0;
            if (_tf) ctx->gpr[2] = _toc0;
            tf(ctx);
        }
        if (g_vm_diag_enabled && g_vcall_sp > 0) g_vcall_sp--;
     ...
     * instead of spinning forever on the same bogus pointer. */
    {
        ...
        if (_invalid) {
            static int _gn = 0;
            if (_gn++ < 12) {
                fprintf(stderr, "[ppu] garbage vcall -> 0x%08X this=0x%08X (uninit/stale object) -- no-op, r3=0\n", tgt, (uint32_t)ctx->gpr[3]);
                ppu_report_free_of((uint32_t)ctx->gpr[3], "this");
                ppu_dump_guest_stack(ctx, "gvcall");
            }
#ifdef _WIN32
...
static void ppu_capture_opd_section(const uint8_t* file, uint64_t fsz)
{
    free(s_opd_snapshot);
    s_opd_snapshot = NULL;
    s_opd_snapshot_base = 0;
    s_opd_snapshot_count = 0;

    uint64_t shoff = be64(file + 40);
    uint16_t shentsize = be16(file + 58);
    uint16_t shnum = be16(file + 60);
    uint16_t shstrndx = be16(file + 62);
    if (shentsize < 64 || shstrndx >= shnum || shoff > fsz ||
        (uint64_t)shnum * shentsize > fsz - shoff)
        return;

    const uint8_t* shstr = file + shoff + (uint64_t)shstrndx * shentsize;
    uint64_t names_off = be64(shstr + 24);
    uint64_t names_size = be64(shstr + 32);
    if (names_off > fsz || names_size > fsz - names_off)
        return;

    for (uint16_t i = 0; i < shnum; i++) {
        const uint8_t* sh = file + shoff + (uint64_t)i * shentsize;
        uint32_t name_off = be32(sh + 0);
        if (name_off >= names_size)
            continue;
        const char* name = (const char*)(file + names_off + name_off);
        size_t name_max = (size_t)(names_size - name_off);
        if (!memchr(name, 0, name_max) || strcmp(name, ".opd") != 0)
            continue;

        uint64_t addr = be64(sh + 16);
        uint64_t off = be64(sh + 24);
        uint64_t size = be64(sh + 32);
        if (addr > UINT32_MAX || off > fsz || size > fsz - off || size / 8u > UINT32_MAX)
            return;

        uint32_t count = (uint32_t)(size / 8u);
        PpuOpdSnapshot* snapshot = (PpuOpdSnapshot*)calloc(count, sizeof(*snapshot));
        if (!snapshot)
            return;
        for (uint32_t j = 0; j < count; j++) {
            snapshot[j].code = be32(file + off + (uint64_t)j * 8u);
            snapshot[j].toc  = be32(file + off + (uint64_t)j * 8u + 4u);
        }
        s_opd_snapshot = snapshot;
        s_opd_snapshot_base = (uint32_t)addr;
        s_opd_snapshot_count = count;
        fprintf(stderr, "[ppu] captured .opd 0x%08X (%u descriptors)\n",
                s_opd_snapshot_base, s_opd_snapshot_count);
        return;
    }
}
...
extern "C" uint32_t ppu_load_elf(const char* path)
{
    ...
    ppu_capture_opd_section(file, fsz);
    free(file);
...
/* Compact load image generated for static-recompilation releases. The native
 * executable contains every lifted PPU function, but this title also reads
 * bytes from its executable PT_LOAD as data. Preserve the exact load segments
 * and omit only non-loadable ELF container/debug data. All integer fields are
 * little-endian; the release target is Windows x86-64. */
#pragma pack(push, 1)
struct PpuEmbeddedImageHeader {
    char magic[8];
    uint32_t version;
...
#pragma pack(pop)
static_assert(sizeof(PpuEmbeddedImageHeader) == 44, "embedded image header ABI");
static_assert(sizeof(PpuEmbeddedImageRange) == 16, "embedded image range ABI");

extern "C" uint32_t ppu_load_embedded_image(const void* image_data, size_t image_size)
{
    if (!image_data || image_size < sizeof(PpuEmbeddedImageHeader)) {
        fprintf(stderr, "[ppu] embedded image is missing or truncated\n");
        return 0;
    }
    const uint8_t* image = (const uint8_t*)image_data;
    const PpuEmbeddedImageHeader* header =
        (const PpuEmbeddedImageHeader*)image;
    if (memcmp(header->magic, "TAIKOIMG", 8) != 0 || header->version != 1 ||
        header->header_size != sizeof(*header)) {
        fprintf(stderr, "[ppu] embedded image has an unsupported format\n");
        return 0;
    }
    const uint64_t table_end = (uint64_t)header->header_size +
        (uint64_t)header->range_count * sizeof(PpuEmbeddedImageRange);
    if (header->range_count > 4096 || table_end > image_size) {
        fprintf(stderr, "[ppu] embedded image range table is invalid\n");
        return 0;
    }

    const PpuEmbeddedImageRange* ranges =
        (const PpuEmbeddedImageRange*)(image + header->header_size);
    uint64_t copied = 0;
    for (uint32_t i = 0; i < header->range_count; ++i) {
        const PpuEmbeddedImageRange& range = ranges[i];
        if (range.filesz > range.memsz ||
            (uint64_t)range.vaddr + range.memsz > 0x100000000ull ||
            (range.filesz && ((uint64_t)range.file_offset + range.filesz > image_size ||
                              range.file_offset < table_end))) {
            fprintf(stderr, "[ppu] embedded range %u is invalid\n", i);
            return 0;
        }
        if (ppu_vm_size &&
            (uint64_t)range.vaddr + range.memsz > ppu_vm_size) {
            fprintf(stderr, "[ppu] embedded range %u exceeds guest VM\n", i);
            return 0;
        }
        if (range.filesz)
            memcpy(vm_base + range.vaddr, image + range.file_offset, range.filesz);
        if (range.memsz > range.filesz)
            memset(vm_base + range.vaddr + range.filesz, 0,
                   range.memsz - range.filesz);
        copied += range.filesz;
    }

    g_tls_vaddr = header->tls_vaddr;
    g_tls_filesz = header->tls_filesz;
    g_tls_memsz = header->tls_memsz;

    free(s_opd_snapshot);
    s_opd_snapshot = NULL;
    s_opd_snapshot_base = header->opd_vaddr;
    s_opd_snapshot_count = header->opd_count;
    if (!header->opd_vaddr || !header->opd_count ||
        (uint64_t)header->opd_vaddr + (uint64_t)header->opd_count * 8u >
            0x100000000ull) {
        fprintf(stderr, "[ppu] embedded image OPD metadata is invalid\n");
        return 0;
    }
    s_opd_snapshot = (PpuOpdSnapshot*)calloc(header->opd_count,
                                             sizeof(*s_opd_snapshot));
    if (!s_opd_snapshot) {
        fprintf(stderr, "[ppu] cannot allocate embedded OPD snapshot\n");
        return 0;
    }
    for (uint32_t i = 0; i < header->opd_count; ++i) {
        const uint8_t* descriptor = vm_base + header->opd_vaddr + i * 8u;
        s_opd_snapshot[i].code = be32(descriptor);
        s_opd_snapshot[i].toc = be32(descriptor + 4);
    }

    fprintf(stderr,
            "[ppu] loaded embedded process image: %u ranges, %.2f MiB, "
            "entry OPD 0x%08X, TLS 0x%08X+0x%X/0x%X\n",
            header->range_count, copied / 1048576.0, header->entry,
            g_tls_vaddr, g_tls_filesz, g_tls_memsz);
    return header->entry;
}
...
static void ppu_thread_entry_trampoline(ppu_context* ctx)
{
    ...
    /* Own TLS block per thread -- see ppu_tls_alloc. Sharing one block lets
     * threads corrupt each other's thread-local state (here: the guest's
     * matrix stack). */
    if (!ctx->gpr[13]) ctx->gpr[13] = ppu_tls_alloc(0, 0, 0);
    ppu_fn fn = ppu_lookup(code);
    if (!fn && ppu_opd_snapshot_resolve((uint32_t)ctx->cia, &code, &toc)) {
        fn = ppu_lookup(code);
        if (fn) {
            ctx->gpr[2] = toc;
            fprintf(stderr, "[ppu] thread OPD 0x%08X restored from ELF -> code=0x%08X toc=0x%08X\n",
                    (uint32_t)ctx->cia, code, toc);
        }
    }
    if (!fn) {
                                   ...
                                   uint64_t a0, uint64_t a1, uint64_t a2, uint64_t a3)
{
    ...
    ppu_fn fn = ppu_lookup(code);
    if (!fn && ppu_opd_snapshot_resolve(opd_addr, &code, &toc))
        fn = ppu_lookup(code);
...
extern "C" int ppu_run(uint32_t entry_opd, uint32_t stack_top)
{
    ...
    ctx.thread_id = 1;             /* host continuation slot / syscall identity */
    ...
    g_active_ctx = &ctx;
    ppu_resv_register(&ctx);   /* main/entry thread joins the reservation set too */
    /* Diagnostic for writes that bypass vm_write* (dcbz, bulk HLE copies, and
     * vector stores). The guard is armed only after the ELF and loader scratch
     * data have been installed, so its first report belongs to guest startup. */
    if (const char* guard = getenv("PPU_GUARD_PAGE"))
        ppu_guard_page((uint32_t)strtoul(guard, NULL, 0));
    fn(&ctx);
    while (ppu_trampoline_for(&ctx)) { void (*tf)(void*) = ppu_trampoline_for(&ctx); ppu_trampoline_for(&ctx) = 0; tf(&ctx); }
    return 0;
}
