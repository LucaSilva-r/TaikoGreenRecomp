...
#ifdef _WIN32
/* Lock-free mirror of accessible guest pages, maintained for the SPU DMA hot
 * path. Definitions live with the PPU loader, which owns the flat guest VM. */
void vm_note_accessible_range(uint32_t addr, uint32_t size, int accessible);
int  vm_range_is_accessible(uint32_t addr, uint32_t size);
void vm_reset_accessible_ranges(void);
#endif
...
static inline int32_t vm_init(void)
{
    ...
    vm_note_accessible_range(VM_MAIN_MEM_BASE, VM_MAIN_MEM_SIZE, 1);
    ...
    vm_note_accessible_range(VM_STACK_BASE, VM_STACK_REGION, 1);
...
static inline int32_t vm_commit(uint32_t addr, uint32_t size)
{
    ...
    if (!VirtualAlloc(vm_base + addr, size, MEM_COMMIT, PAGE_READWRITE))
        return CELL_ENOMEM;
    vm_note_accessible_range(addr, size, 1);
#else
...
static inline int32_t vm_protect(uint32_t addr, uint32_t size, int read, int write, int exec)
{
    ...
    if (!VirtualProtect(vm_base + addr, size, prot, &old))
        return CELL_EFAULT;
    vm_note_accessible_range(addr, size, prot != PAGE_NOACCESS);
#else
    int prot = PROT_NONE;
