...
typedef struct spu_channel {
    uint32_t values[4];
    volatile uint32_t count; /* inbound mailbox can hold four; others use one */
...
typedef struct spu_context {
    ...
    /* Raw SPU threads may block on inbound mailboxes/signals while their PPU
     * service thread handles a stop-and-signal request. Lifted one-shot jobs
     * leave this disabled and retain the historical nonblocking behavior. */
    volatile uint32_t channel_blocking;
    volatile uint32_t channel_shutdown;
...
/* Local-store write watch: SPU_LSWATCH=<hex LS address> logs every quadword
 * store covering it, with the SPU pc. Guest memory has YDKJ_AWATCH; LS-local
 * state (working fields the SPU computes and never DMAs back) had no
 * equivalent, which made "who sets this field" unanswerable for SPU code. */
static inline void spu_ls_write128(spu_context* ctx, uint32_t lsa, u128 val)
{
    lsa &= SPU_LS_MASK & ~0xFu;
    {
        ...
        if (watch < 0) {
            const char* e = getenv("SPU_LSWATCH");
            addr = e ? (uint32_t)strtoul(e, NULL, 16) : 0;
            watch = e ? 1 : 0;
        }
...
/* Append to the four-entry inbound mailbox. Callers must not push when full. */
static inline void spu_channel_push(spu_channel* ch, uint32_t val)
{
    ch->values[ch->tail & 3u] = val;
    ch->tail = (ch->tail + 1u) & 3u;
    ch->count++;
}

static inline uint32_t spu_channel_read(spu_channel* ch)
{
    uint32_t val = ch->values[ch->head & 3u];
    if (ch->count) {
        ch->head = (ch->head + 1u) & 3u;
        ch->count--;
    }
    return val;
}
