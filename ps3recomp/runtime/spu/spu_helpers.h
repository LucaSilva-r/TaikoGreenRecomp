...
/* Float<->int conversions with the instruction's unsigned RI8 scale.
 * RPCS3's precise interpreter and ASMJIT recompiler implement:
 *   CFLTS/CFLTU: value * 2^(173 - i8)
 *   CSFLT/CUFLT: value * 2^(i8 - 155)
 * The former code subtracted an additional 155 from both exponents (and used
 * the opposite direction for int->float).  Thus CSFLT(256, 155), which must
 * be 256.0f, underflowed to zero; bnusCore immediately took its reciprocal
 * and filled the audio mix with the SPU divide-by-zero pattern 0x7FFFFC00. */
static inline u128 spu_cflts(u128 a, int i8){ u128 r; float f=exp2f((float)(173-i8));
...
static inline u128 spu_cfltu(u128 a, int i8){ u128 r; float f=exp2f((float)(173-i8));
    for(int i=0;i<4;i++){ double v=(double)a._f32[i]*f; if(v<0)v=0; if(v>4294967295.0)v=4294967295.0; r._u32[i]=(uint32_t)v; } return r; }
static inline u128 spu_csflt(u128 a, int i8){ u128 r; float f=exp2f((float)(i8-155));
    for(int i=0;i<4;i++) r._f32[i]=(float)a._s32[i]*f; return r; }
static inline u128 spu_cuflt(u128 a, int i8){ u128 r; float f=exp2f((float)(i8-155));
...
/* xsbh/xshw take the low sub-element of each slot. Our u128 is host-native
 * per 32-bit word, so within a word the low halfword/byte already sits at the
 * even _s16/_u8 index -- these are correct as written. */
...
/* XSWD sign-extends the LOW word of each doubleword. It cannot go through
 * _s64: a doubleword spans two _u32 slots, and since _u32[i] holds SPU word i
 * while the host packs _u64 little-endian, _s64[d] has the two SPU words in
 * the opposite order. The old version therefore read the HIGH word (_s32[2d])
 * and deposited the value back into the HIGH word, so a 64-bit value built as
 * {0, ea} -- the standard way to form a DMA effective address, and what
 * bnusCore's audio mixer does for every PCM fetch -- extended to all zeros.
 * Every source DMA then targeted EA 0 and was dropped, so the mixer produced
 * pure silence. Operate on the SPU word slots explicitly. */
static inline u128 spu_xswd(u128 a) {
    u128 r;
    for (int d = 0; d < 2; d++) {
        int32_t v = a._s32[2*d + 1];                      /* low word of dword d */
        r._u32[2*d]     = (v < 0) ? 0xFFFFFFFFu : 0u;     /* high word = sign */
        r._u32[2*d + 1] = (uint32_t)v;                    /* low word = value */
    }
    return r;
}
