...
/* Raw SPU system-call bridge. A returning hook means the stop was serviced and
 * execution resumes at the following instruction; other stop codes retain the
...
void spu_stop(spu_context* ctx)
{
    ...
    if (g_spu_stop_hook && g_spu_stop_hook(ctx, ctx->stop_code)) {
...
/* Global spinlock guarding all atomic line ops. */
static volatile long g_resv_lock = 0;
static void resv_lock(void)
{
    while (__atomic_exchange_n(&g_resv_lock, 1, __ATOMIC_ACQUIRE))
        ps3_host_cpu_relax();
}
static void resv_unlock(void)
{
    __atomic_store_n(&g_resv_lock, 0, __ATOMIC_RELEASE);
}
...
u128 spu_rdch(spu_context* ctx, uint32_t channel)
{
    ...
    if (ctx->channel_blocking &&
        (channel == SPU_RdInMbox || channel == SPU_RdSigNotify1 ||
         channel == SPU_RdSigNotify2)) {
        spu_channel* ch = channel == SPU_RdInMbox ? &ctx->ch_in_mbox :
                          &ctx->ch_sig_notify[channel - SPU_RdSigNotify1];
        while (!ch->count && !ctx->channel_shutdown) {
#ifdef _WIN32
            Sleep(1);
#else
...
/* A --trace lift logs every instruction, and an SPU running far faster than
 * realtime fills a disk in seconds. SPU_TRACE_TRIG=<hex LS pc> holds the trace
 * off until that address first executes, and SPU_TRACE_LIMIT=<n> caps how many
 * instructions are logged after it (default 200000), which is what makes a
 * bounded window around one divergence practical. Unset SPU_TRACE_TRIG traces
 * from the first instruction. */
static long     s_trace_budget = -1;
static uint32_t s_trace_trig   = 0;
static int      s_trace_on     = 0;

void spu_trace_pc(spu_context* ctx, uint32_t pc)
{
    (void)ctx;
    if (!s_trace_fp) spu_trace_init(getenv("SPU_TRACE_FILE"));
    pc &= SPU_LS_MASK;

    if (s_trace_budget < 0) {
        const char* t = getenv("SPU_TRACE_TRIG");
        const char* n = getenv("SPU_TRACE_LIMIT");
        s_trace_trig = t ? (uint32_t)strtoul(t, NULL, 16) : 0;
        s_trace_budget = n ? strtol(n, NULL, 0) : 200000;
        s_trace_on = s_trace_trig ? 0 : 1;
    }
    if (!s_trace_on) {
        if (pc != s_trace_trig) return;
        s_trace_on = 1;
        fprintf(s_trace_fp, "# trace armed at %05X\n", pc);
    }
    if (s_trace_budget <= 0) { s_trace_on = 0; return; }
    s_trace_budget--;
...
{
    if (!s_trace_on || s_trace_budget <= 0) return;
    if (!s_trace_fp) s_trace_fp = stderr;
