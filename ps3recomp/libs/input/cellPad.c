 ...
 * -----------------------------------------------------------------------*/
...
#if defined(PS3RECOMP_INPUT_BACKEND_NULL)
  #define PAD_BACKEND_SDL2  0
  #define PAD_BACKEND_SDL3  0
  #define PAD_BACKEND_XINPUT 0
  #define PAD_BACKEND_NULL 1
#elif defined(PS3RECOMP_PAD_USE_SDL2)
  #define PAD_BACKEND_SDL2  1
  #define PAD_BACKEND_SDL3  0
  #define PAD_BACKEND_XINPUT 0
  #define PAD_BACKEND_NULL 0
#elif defined(PS3RECOMP_INPUT_BACKEND_WIN32)
  #define PAD_BACKEND_SDL2  0
  #define PAD_BACKEND_SDL3  0
  #define PAD_BACKEND_XINPUT 1
  #define PAD_BACKEND_NULL 0
#elif defined(PS3RECOMP_INPUT_BACKEND_SDL2)
  #define PAD_BACKEND_SDL2  1
  #define PAD_BACKEND_SDL3  0
  #define PAD_BACKEND_XINPUT 0
  #define PAD_BACKEND_NULL 0
  ...
  #define PAD_BACKEND_SDL2  0
  #define PAD_BACKEND_SDL3  1
  #define PAD_BACKEND_XINPUT 0
  #define PAD_BACKEND_NULL 0
#else
  #error "No supported ps3recomp pad backend selected"
#endif
...
#endif
#if PAD_BACKEND_SDL3
static SDL_Gamepad* s_sdl3_gamepads[PAD_MAX_HOST_PORTS];
#endif
...
static u8 pad_sdl3_axis_to_u8(int raw)
{
    int val = ((raw + 32768) * 255) / 65535;
    if (val < 0) val = 0;
    if (val > 255) val = 255;
...
static u8 pad_sdl3_trigger_to_u8(int raw)
{
    int val = (raw * 255) / 32767;
    if (val < 0) val = 0;
    if (val > 255) val = 255;
...
static void pad_sdl3_open_available(void)
{
    int count = 0;
    SDL_JoystickID* ids = SDL_GetGamepads(&count);
    for (int source = 0; source < count; ++source) {
        int already_open = 0;
        for (int slot = 0; slot < PAD_MAX_HOST_PORTS; ++slot)
            if (s_sdl3_gamepads[slot] &&
                SDL_GetGamepadID(s_sdl3_gamepads[slot]) == ids[source])
                already_open = 1;
        if (already_open) continue;
        for (int slot = 0; slot < PAD_MAX_HOST_PORTS; ++slot) {
            if (!s_sdl3_gamepads[slot]) {
                s_sdl3_gamepads[slot] = SDL_OpenGamepad(ids[source]);
                break;
            }
        }
    }
...
static void pad_poll_sdl3(void)
{
    SDL_UpdateGamepads();
    pad_sdl3_open_available();
    for (int i = 0; i < PAD_MAX_HOST_PORTS; ++i) {
        SDL_Gamepad* gp = s_sdl3_gamepads[i];
        if (!gp || !SDL_GamepadConnected(gp)) {
            s_host_state[i].connected = 0;
            if (gp) SDL_CloseGamepad(gp);
            s_sdl3_gamepads[i] = NULL;
            continue;
        }
        s_host_state[i].connected = 1;
#define GP_BUTTON(name, bit) \
        if (SDL_GetGamepadButton(gp, SDL_GAMEPAD_BUTTON_##name)) btns |= bit
        u16 btns = 0;
        GP_BUTTON(BACK, CELL_PAD_CTRL_SELECT);
        GP_BUTTON(LEFT_STICK, CELL_PAD_CTRL_L3);
        GP_BUTTON(RIGHT_STICK, CELL_PAD_CTRL_R3);
        GP_BUTTON(START, CELL_PAD_CTRL_START);
        GP_BUTTON(DPAD_UP, CELL_PAD_CTRL_UP);
        GP_BUTTON(DPAD_RIGHT, CELL_PAD_CTRL_RIGHT);
        GP_BUTTON(DPAD_DOWN, CELL_PAD_CTRL_DOWN);
        GP_BUTTON(DPAD_LEFT, CELL_PAD_CTRL_LEFT);
        GP_BUTTON(LEFT_SHOULDER, CELL_PAD_CTRL_L1);
        GP_BUTTON(RIGHT_SHOULDER, CELL_PAD_CTRL_R1);
        GP_BUTTON(NORTH, CELL_PAD_CTRL_TRIANGLE);
        GP_BUTTON(EAST, CELL_PAD_CTRL_CIRCLE);
        GP_BUTTON(SOUTH, CELL_PAD_CTRL_CROSS);
        GP_BUTTON(WEST, CELL_PAD_CTRL_SQUARE);
#undef GP_BUTTON
        int left_trigger = SDL_GetGamepadAxis(gp, SDL_GAMEPAD_AXIS_LEFT_TRIGGER);
        int right_trigger = SDL_GetGamepadAxis(gp, SDL_GAMEPAD_AXIS_RIGHT_TRIGGER);
        if (left_trigger > 3000) btns |= CELL_PAD_CTRL_L2;
        if (right_trigger > 3000) btns |= CELL_PAD_CTRL_R2;
        s_host_state[i].buttons = btns;
        s_host_state[i].analog_lx = pad_sdl3_axis_to_u8(
            SDL_GetGamepadAxis(gp, SDL_GAMEPAD_AXIS_LEFTX));
        s_host_state[i].analog_ly = pad_sdl3_axis_to_u8(
            SDL_GetGamepadAxis(gp, SDL_GAMEPAD_AXIS_LEFTY));
        s_host_state[i].analog_rx = pad_sdl3_axis_to_u8(
            SDL_GetGamepadAxis(gp, SDL_GAMEPAD_AXIS_RIGHTX));
        s_host_state[i].analog_ry = pad_sdl3_axis_to_u8(
            SDL_GetGamepadAxis(gp, SDL_GAMEPAD_AXIS_RIGHTY));
        s_host_state[i].trigger_l2 = pad_sdl3_trigger_to_u8(left_trigger);
        s_host_state[i].trigger_r2 = pad_sdl3_trigger_to_u8(right_trigger);
        s_host_state[i].press_up = (btns & CELL_PAD_CTRL_UP) ? 255 : 0;
        s_host_state[i].press_down = (btns & CELL_PAD_CTRL_DOWN) ? 255 : 0;
        s_host_state[i].press_left = (btns & CELL_PAD_CTRL_LEFT) ? 255 : 0;
        s_host_state[i].press_right = (btns & CELL_PAD_CTRL_RIGHT) ? 255 : 0;
...
static void pad_init_backend(void)
{
    memset(s_sdl3_gamepads, 0, sizeof(s_sdl3_gamepads));
    pad_sdl3_open_available();
}
...
{
    for (int i = 0; i < PAD_MAX_HOST_PORTS; ++i) {
        if (s_sdl3_gamepads[i]) SDL_CloseGamepad(s_sdl3_gamepads[i]);
        s_sdl3_gamepads[i] = NULL;
    }
}

#endif /* PAD_BACKEND_SDL3 */
...
void cellPadHostShutdown(void)
{
    if (!s_pad_initialized) return;
    fprintf(stderr, "[cellPad] host shutdown\n");
    pad_shutdown_backend();
    s_pad_initialized = 0;
    s_max_connect = 0;
}
