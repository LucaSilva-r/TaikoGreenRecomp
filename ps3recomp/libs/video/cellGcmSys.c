...
/* Host frame-driver query. A completed frame must remain sealed until the
 * vblank-paced present consumes it; draining beyond its flip would append the
 * next frame to the recorder batch. */
int cellGcm_flip_is_pending(void)
{
    return s_flip_pending != 0;
}
...
#   define GCM_SLEEP_1MS() Sleep(1)
#else
#   include <time.h>
#   define GCM_SLEEP_1MS() nanosleep(&(struct timespec){0, 1000000L}, NULL)
#endif
...
{
    return (unsigned long long)ps3_host_monotonic_ms();
}

static u32 s_fifo_getoff  = 0;

/* Trace probe: FIFO ring offset the drain is currently executing from. The
 * backend stamps this onto each recorded draw so the replay order can be
 * checked against the order the commands actually sit at in the ring. */
u32 cellGcm_current_fifo_getoff(void) { return s_fifo_getoff; }
...
/* FIFO subchannels are dynamically assigned with SET_OBJECT.  Several SDK
 * revisions use different slots (Taiko binds the 3D object on subchannel 7),
 * so routing solely by subchannel number drops valid NV4097 methods and can
 * mistake transfer commands for another 2D engine. */
#define GCM_OBJ_3D             0x31337000u
#define GCM_OBJ_M2MF           0x31337303u
#define GCM_OBJ_SURFACE_2D     0x313371C3u
#define GCM_OBJ_IMAGE_CPU      0x31337808u
#define GCM_OBJ_SURFACE_SWZ    0x31337A73u
#define GCM_OBJ_SCALED_IMAGE   0x3137AF00u
static u32 s_gcm_subch_obj[8] = { GCM_OBJ_3D, 0, 0, 0, 0, 0, 0, 0 };

static struct {
    u32 src_dma, dst_dma;
    u32 src_offset, dst_offset;
    s32 src_pitch, dst_pitch;
    u32 line_length, line_count;
    u32 format;
} s_gcmm2mf;

static void gcm_m2mf_copy(void)
{
    if (!s_gcmm2mf.line_length || !s_gcmm2mf.line_count) return;
    int src_local = (s_gcmm2mf.src_dma != 0xFEED0001u);
    int dst_local = (s_gcmm2mf.dst_dma != 0xFEED0001u);
    u32 src_ea = cellGcmResolveLocated(src_local, s_gcmm2mf.src_offset);
    u32 dst_ea = cellGcmResolveLocated(dst_local, s_gcmm2mf.dst_offset);
    if (!src_ea || !dst_ea) return;

    /* FORMAT's low/high bytes are source/destination byte strides. Normal
     * texture uploads use 1/1. Keep the uncommon gather/scatter form too. */
    u32 src_stride = s_gcmm2mf.format & 0xFFu;
    u32 dst_stride = (s_gcmm2mf.format >> 8) & 0xFFu;
    if (!src_stride) src_stride = 1;
    if (!dst_stride) dst_stride = 1;

    u64 tmp_size = (u64)s_gcmm2mf.line_length * s_gcmm2mf.line_count;
    u8* tmp = (u8*)malloc((size_t)tmp_size);
    if (!tmp) return;
    for (u32 y = 0; y < s_gcmm2mf.line_count; y++) {
        const u8* src = vm_base + src_ea + (s64)y * s_gcmm2mf.src_pitch;
        u8* row = tmp + (u64)y * s_gcmm2mf.line_length;
        if (src_stride == 1) memcpy(row, src, s_gcmm2mf.line_length);
        else for (u32 x = 0; x < s_gcmm2mf.line_length; x++) row[x] = src[x * src_stride];
    }
    for (u32 y = 0; y < s_gcmm2mf.line_count; y++) {
        u8* dst = vm_base + dst_ea + (s64)y * s_gcmm2mf.dst_pitch;
        const u8* row = tmp + (u64)y * s_gcmm2mf.line_length;
        if (dst_stride == 1) memcpy(dst, row, s_gcmm2mf.line_length);
        else for (u32 x = 0; x < s_gcmm2mf.line_length; x++) dst[x * dst_stride] = row[x];
    }
    free(tmp);
    { static int _m2 = 0; if (_m2++ < (getenv("GCM2D_PROBE") ? 400 : 12))
        printf("[GCM2D] m2mf copy src=0x%08X dst=0x%08X len=%u lines=%u pitch=%d/%d fmt=0x%08X\n",
               src_ea, dst_ea, s_gcmm2mf.line_length, s_gcmm2mf.line_count,
               s_gcmm2mf.src_pitch, s_gcmm2mf.dst_pitch, s_gcmm2mf.format); }
}

static void gcm_2d_method(u32 subch, u32 object, u32 method, u32 data)
{
    if (getenv("GCM2D_TRACE")) {
        static u32 trace_n;
        if (trace_n++ < 20000)
            fprintf(stderr,
                    "[GCM2D-TRACE] n=%u fifo=0x%08X subch=%u obj=0x%08X method=0x%04X data=0x%08X\n",
                    trace_n, s_fifo_getoff, subch, object, method, data);
    }
    /* GCM2D_PROBE: every distinct (subch, method) that reaches the 2D path.
     * The routing below claims subch 4/5 for IMAGE_CPU when no SET_OBJECT was
     * seen, so NV3089 (scaled image, classic subch 5) is swallowed without
     * ever hitting the "unhandled" warning at the bottom. Log here instead. */
    if (getenv("GCM2D_PROBE")) {
        static u32 seen[128];
        static u32 seen_n;
        u32 key = (subch << 16) | method;
        int dup = 0;
        for (u32 i = 0; i < seen_n; i++) if (seen[i] == key) { dup = 1; break; }
        if (!dup && seen_n < 128) {
            seen[seen_n++] = key;
            printf("[GCM2D-PROBE] subch=%u obj=0x%08X method=0x%04X data=0x%08X\n",
                   subch, object, method, data);
        }
    }
    ...
    int known = object != 0;
    if (object == GCM_OBJ_M2MF || (!known && (subch == 1 || subch == 6))) {
        switch (method) {
        case 0x0184: s_gcmm2mf.src_dma     = data; return;
        case 0x0188: s_gcmm2mf.dst_dma     = data; return;
        case 0x030C: s_gcmm2mf.src_offset  = data; return;
        case 0x0310: s_gcmm2mf.dst_offset  = data; return;
        case 0x0314: s_gcmm2mf.src_pitch   = (s32)data; return;
        case 0x0318: s_gcmm2mf.dst_pitch   = (s32)data; return;
        case 0x031C: s_gcmm2mf.line_length = data; return;
        case 0x0320: s_gcmm2mf.line_count  = data; return;
        case 0x0324: s_gcmm2mf.format      = data; return;
        case 0x0328: gcm_m2mf_copy(); return; /* BUFFER_NOTIFY executes copy */
        }
        return;
    }
    if (object == GCM_OBJ_SURFACE_2D || (!known && (subch == 2 || subch == 3))) {
        switch (method) {
    ...
    if (object == GCM_OBJ_IMAGE_CPU || (!known && (subch == 4 || subch == 5))) {
        switch (method) {
    ...
    static int warned = 0;
    if (warned++ < 8)
        printf("[cellGcmSys] FIFO subch %u obj=0x%08X method 0x%04X (unhandled 2D engine)\n",
               subch, object, method);
}
...
void cellGcm_rsx_process_fifo(void)
{
    ...
    static u64 s_prof_start_ns, s_prof_time_ns, s_prof_max_ns;
    static u64 s_prof_calls, s_prof_busy, s_prof_headers, s_prof_methods;
    static u64 s_prof_budget_hits;
    static u64 s_prof_begin_ns, s_prof_begin_calls;
    static u64 s_prof_draw_ns, s_prof_draw_calls;
    static u64 s_prof_method_hist[0x800];
    if (s_prof_enabled < 0) {
        const char* e = getenv("RSX_PROFILE");
        s_prof_enabled = (e && e[0] != '0') ? 1 : 0;
    }
    u64 prof_t0 = s_prof_enabled ? get_timestamp_ns() : 0;
    ...
    int prof_was_busy = (s_fifo_getoff != put);
    u64 prof_methods = 0;
    while (s_fifo_getoff != put && budget-- > 0) {
        ...
        if (type == 0 || type == 2) {          /* method (incrementing / NI) */
            ...
            for (u32 i = 0; i < count; i++) {
                ...
                if (s_prof_enabled && (m >> 2) < 0x800)
                    s_prof_method_hist[m >> 2]++;
                u32 data = vm_read32(dea);
                if (m == 0) { /* SET_OBJECT: bind an engine to this subchannel */
                    s_gcm_subch_obj[subch] = data;
                    { static int _ob = 0; if (_ob++ < 16)
                        printf("[GCM] subchannel %u SET_OBJECT 0x%08X\n", subch, data); }
                } else if (s_gcm_subch_obj[subch] == GCM_OBJ_3D ||
                           (s_gcm_subch_obj[subch] == 0 && subch == 0) ||
                           /* Some command rings inherit the device's binding
                            * before our drain begins. CONTROL2 is unambiguously
                            * an NV4097 method, so recover that inherited bind. */
                           (m >= 0x0B00 && m <= 0x0B3C)) {
                    if (m >= 0x0B00 && m <= 0x0B3C)
                        s_gcm_subch_obj[subch] = GCM_OBJ_3D;
                    int prof_kind = s_prof_enabled && m == NV4097_SET_BEGIN_END ? 1 :
                                    s_prof_enabled && (m == NV4097_DRAW_ARRAYS ||
                                                       m == NV4097_DRAW_INDEX_ARRAY) ? 2 : 0;
                    u64 prof_method_t0 = prof_kind ? get_timestamp_ns() : 0;
                    rsx_process_method(&s_state, m, data);
                    if (prof_kind) {
                        u64 prof_method_dt = get_timestamp_ns() - prof_method_t0;
                        if (prof_kind == 1) {
                            s_prof_begin_ns += prof_method_dt;
                            s_prof_begin_calls++;
                        } else {
                            s_prof_draw_ns += prof_method_dt;
                            s_prof_draw_calls++;
                        }
                    }
                } else {
                    gcm_2d_method(subch, s_gcm_subch_obj[subch], m, data);
                }
            }
    ...
    if (s_prof_enabled) {
        u64 t1 = get_timestamp_ns();
        u64 dt = t1 - prof_t0;
        u64 headers = (u64)(0x100000 - budget);
        s_prof_calls++;
        s_prof_busy += prof_was_busy ? 1 : 0;
        s_prof_headers += headers;
        s_prof_methods += prof_methods;
        s_prof_time_ns += dt;
        if (dt > s_prof_max_ns) s_prof_max_ns = dt;
        if (budget <= 0) s_prof_budget_hits++;
        if (!s_prof_start_ns) s_prof_start_ns = t1;
        if (t1 - s_prof_start_ns >= 1000000000ULL) {
            double elapsed_s = (double)(t1 - s_prof_start_ns) / 1.0e9;
            fprintf(stderr,
                    "[FIFOPROF] calls=%.1f/s busy=%.1f/s headers=%.0f/s methods=%.0f/s "
                    "cpu=%.3fms/s avg_busy=%.3fms max=%.3fms budget=%llu\n",
                    s_prof_calls / elapsed_s, s_prof_busy / elapsed_s,
                    s_prof_headers / elapsed_s, s_prof_methods / elapsed_s,
                    s_prof_time_ns / 1.0e6 / elapsed_s,
                    s_prof_busy ? s_prof_time_ns / 1.0e6 / s_prof_busy : 0.0,
                    s_prof_max_ns / 1.0e6,
                    (unsigned long long)s_prof_budget_hits);
            fprintf(stderr,
                    "[FIFOPROF] callbacks begin=%.3fms/s %.2fus/call (%llu) "
                    "draw=%.3fms/s %.2fus/call (%llu)\n",
                    s_prof_begin_ns / 1.0e6 / elapsed_s,
                    s_prof_begin_calls ? s_prof_begin_ns / 1.0e3 / s_prof_begin_calls : 0.0,
                    (unsigned long long)s_prof_begin_calls,
                    s_prof_draw_ns / 1.0e6 / elapsed_s,
                    s_prof_draw_calls ? s_prof_draw_ns / 1.0e3 / s_prof_draw_calls : 0.0,
                    (unsigned long long)s_prof_draw_calls);
            fprintf(stderr, "[FIFOPROF] top methods:");
            for (unsigned rank = 0; rank < 12; ++rank) {
                u32 best = 0;
                for (u32 i = 1; i < 0x800; ++i)
                    if (s_prof_method_hist[i] > s_prof_method_hist[best]) best = i;
                if (!s_prof_method_hist[best]) break;
                fprintf(stderr, " %04X=%.0f/s", best << 2,
                        s_prof_method_hist[best] / elapsed_s);
                s_prof_method_hist[best] = 0;
            }
            fprintf(stderr, "\n");
            s_prof_start_ns = t1;
            s_prof_time_ns = s_prof_max_ns = 0;
            s_prof_calls = s_prof_busy = s_prof_headers = s_prof_methods = 0;
            s_prof_budget_hits = 0;
            s_prof_begin_ns = s_prof_begin_calls = 0;
            s_prof_draw_ns = s_prof_draw_calls = 0;
            memset(s_prof_method_hist, 0, sizeof(s_prof_method_hist));
        }
    }
}
...
void cellGcm_fifo_recycle(u32 ctx_ea)
{
    ...
    int spins = 0;
    while (g_gcm_fifo_drained_ea != begin && spins < 2000) {
        ps3_host_sleep_ms(1);
        spins++;
    }
    if (spins >= 2000) {
 ...
 * use this. `local` != 0 -> local memory.  A MAIN lookup must consult the IO
 * table before the ambiguous local-page hints: the two offset spaces can have
 * the same page number, and the caller has already disambiguated them. */
u32 cellGcmResolveLocated(int local, u32 offset)
{
    ...
    /* Keep the historical fallback for titles that submit an unmapped MAIN
     * offset.  Valid MAIN buffers take the table path above. */
