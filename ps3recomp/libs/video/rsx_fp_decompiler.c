...
static int instruction_has_inline_constant(u32 w1, u32 w2, u32 w3)
{
    return !(w2 & FP_BRANCH) &&
...
u32 rsx_fp_program_size(const u8* ucode, u32 max_bytes)
{
    ...
    while (off + 16 <= max_bytes) {
        ...
        if (instruction_has_inline_constant(w1, w2, w3)) {
...
u64 rsx_fp_program_structure_hash(const u8* ucode, u32 max_bytes)
{
    if (!ucode) return 0;
    ...
    while (off + 16 <= max_bytes) {
        ...
        if (instruction_has_inline_constant(w1, w2, w3) &&
            off + 16 <= max_bytes) {
            /* Preserve the slot in the structure while excluding its mutable
             * payload from shader/pipeline identity. */
            for (u32 i = 0; i < 16; ++i) {
                hash ^= 0;
                hash *= UINT64_C(1099511628211);
            }
        ...
        if (w0 & FP_END) return hash;
    }
    return hash;
}

u32 rsx_fp_extract_constants(const u8* ucode, u32 max_bytes,
                             float* output, u32 max_constants)
{
    if (!ucode) return 0;
    u32 off = 0, count = 0;
    while (off + 16 <= max_bytes) {
        ...
        if (instruction_has_inline_constant(w1, w2, w3) &&
            off + 16 <= max_bytes) {
            if (output && count < max_constants) {
                for (u32 i = 0; i < 4; ++i) {
                    u32 word = rsx_fp_read_word(ucode + off + i * 4);
                    memcpy(output + count * 4u + i, &word, sizeof(word));
                }
            }
            ++count;
            off += 16;
...
static void emit_src(const Src* s, u32 input_src, const float* k, int has_k,
                     int dynamic_constants, u32 constant_index,
                     char* buf, u32 bufsz)
{
    char base[96];
    if (s->type == FP_REG_TYPE_TEMP) {
        ...
        if (has_k && dynamic_constants)
            snprintf(base, sizeof(base), "rsx_fp_constants[%u]",
                     constant_index);
        else if (has_k)
                                ...
                                int dynamic_constants, unsigned color_targets)
{
    ...
    out_puts(&o,
        ...
        "#ifdef RSX_SHADER_REMAP\n"
        "cbuffer FPTex : register(b1) { float4 rsx_texscale[4]; float4 rsx_alphatest; float4 rsx_remap[4]; };\n"
        "float rsx_remap_component(float4 v,float d){return d<0.5?v.x:d<1.5?v.y:d<2.5?v.z:d<3.5?v.w:d<4.5?0.0:1.0;}\n"
        "float4 rsx_apply_remap(float4 v,uint u){float4 d=rsx_remap[u];return float4(rsx_remap_component(v,d.x),rsx_remap_component(v,d.y),rsx_remap_component(v,d.z),rsx_remap_component(v,d.w));}\n"
        "#else\n"
    ...
    while (off + 16 <= max_bytes) {
        ...
        emit_src(&s0, input_src, k, has_k, dynamic_constants,
                 constant_index, a, sizeof(a));
        emit_src(&s1, input_src, k, has_k, dynamic_constants,
                 constant_index, b, sizeof(b));
        emit_src(&s2, input_src, k, has_k, dynamic_constants,
                 constant_index, c, sizeof(c));
        if (has_k) ++constant_index;
        ...
        if (is_branch) {
            ...
            } else if (opcode == 0x05 /* RET = 0x45 & 0x3F */) {
                /* Conditional fragment-program return.  Lumen's silhouette
                 * composite samples the body, returns that colour when alpha
                 * is nonzero, and falls through to its edge detector only for
                 * transparent pixels.  Dropping RET therefore leaves exactly
                 * the outline visible. */
                if (exec_cond != 0) {
                    char bl[220];
                    if (cmp)
        ...
        case OP_TEX:
            snprintf(rhs, sizeof(rhs),
                     "rsx_apply_remap(lerp(float4(1,1,1,1), rsx_tex%u.Sample(rsx_samp%u, (%s).xy * rsx_texscale[%u].xy), rsx_texscale[%u].z),%u)",
                     tex_unit & 3u, tex_unit & 3u, a,
                     tex_unit & 3u, tex_unit & 3u, tex_unit & 3u);
            break;
        case OP_TXP:
            snprintf(rhs, sizeof(rhs),
                     "rsx_apply_remap(lerp(float4(1,1,1,1), rsx_tex%u.Sample(rsx_samp%u, (%s).xy / (%s).w * rsx_texscale[%u].xy), rsx_texscale[%u].z),%u)",
