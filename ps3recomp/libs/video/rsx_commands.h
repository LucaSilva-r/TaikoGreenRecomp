...
#define NV4097_SET_RESTART_INDEX_ENABLE        0x00001DAC
#define NV4097_SET_RESTART_INDEX               0x00001DB0
...
/* Per-unit linear texture pitch. Unlike the other texture registers this is
 * a separate packed array: CONTROL3 + unit * 4. */
...
typedef struct rsx_state {
    ...
    u32 index_array_dma;
    int restart_index_enable;
    u32 restart_index;

    /* DRAW_INDEX_ARRAY may contain several packed ranges inside one
     * BEGIN_END.  They are one hardware draw, not separate strips. */
    u32 pending_index_first;
    u32 pending_index_count;
    int pending_index_draw;
...
typedef struct rsx_backend {
    ...
    /* Complete state snapshot, called exactly once at each nonzero BEGIN_END
     * before the more specific dirty-state notifications below. */
    void (*sync_state)(void* userdata, const rsx_state* state);
