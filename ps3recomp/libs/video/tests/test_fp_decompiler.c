...
static void check_true(const char* test, int condition)
{
    if (condition) {
        printf("[PASS] %s\n", test);
        g_pass++;
...
int main(void)
{
    ...
    /* Test 1: MOV r0, COL0  (write all, END). */
    {
        ...
        put_word(prog + 4, T_INPUT | SWZ_IDENT | EXEC_TR);             /* src0 = INPUT */
        put_word(prog + 8, 0);
        put_word(prog + 12, 0);
        int n = rsx_fp_decompile(prog, sizeof(prog), hlsl, sizeof(hlsl), 1);
        printf("-- test1 MOV: %d instr\n", n);
        check("mov_dest",  hlsl, "r[0].xyzw =");
        check("mov_input", hlsl, "input.col");
        check("mov_return", hlsl, "return _po;");
    }
    ...
    /* Test 2: MUL r1.xy, r0, r0  (partial write mask). */
    {
        ...
        put_word(prog + 4, T_TEMP | REG(0) | SWZ_IDENT | EXEC_TR);
        put_word(prog + 8, T_TEMP | REG(0) | SWZ_IDENT);
        put_word(prog + 12, 0);
        rsx_fp_decompile(prog, sizeof(prog), hlsl, sizeof(hlsl), 1);
    ...
    /* Test 4: TEX r0, TC0 from texture unit 3. */
    {
        ...
        put_word(prog + 4, T_INPUT | SWZ_IDENT | EXEC_TR);
        put_word(prog + 8, 0);
        put_word(prog + 12, 0);
        rsx_fp_decompile(prog, sizeof(prog), hlsl, sizeof(hlsl), 1);
        check("tex_sample", hlsl, "rsx_tex3.Sample(rsx_samp3");
    }
    ...
    /* Test 5: negate + abs on a source (MOV r0, -|r1|). */
    {
        ...
        put_word(prog + 4, T_TEMP | REG(1) | SWZ_IDENT | EXEC_TR |
                           (1u<<17) | (1u<<29)); /* neg+abs */
        put_word(prog + 8, 0);
        put_word(prog + 12, 0);
        rsx_fp_decompile(prog, sizeof(prog), hlsl, sizeof(hlsl), 1);
    ...
    /* Test 6: conditional RET (NV40 branch opcode 0x45). */
    {
        u8 prog[32];
        put_word(prog + 0, OPC(0x01) | OUTMASK_ALL | SET_CC);
        put_word(prog + 4, T_INPUT | SWZ_IDENT | EXEC_TR);
        put_word(prog + 8, 0);
        put_word(prog + 12, 0);
        put_word(prog + 16, OPC(0x05) | END); /* branch-table RET */
        put_word(prog + 20, (5u << 18));      /* if cc0.x != 0 */
        put_word(prog + 24, BRANCH);
        put_word(prog + 28, 0);
        rsx_fp_decompile(prog, sizeof(prog), hlsl, sizeof(hlsl), 1);
        check("ret_condition", hlsl, "if (any(_cs != 0))");
        check("ret_export", hlsl, "return _ret;");
    }

    /* Test 7: inline constant values are dynamic shader data. Their values
     * must not change cache identity, and the SDL variant must reference the
     * extracted uniform slot instead of baking a new HLSL literal. */
    {
        u8 a[32], b[32];
        put_word(a + 0, OPC(0x01) | OUTMASK_ALL | END);
        put_word(a + 4, T_CONST | SWZ_IDENT | EXEC_TR);
        put_word(a + 8, 0); put_word(a + 12, 0);
        memcpy(b, a, 16);
        put_float(a + 16, 0.25f); put_float(a + 20, 0.5f);
        put_float(a + 24, 0.75f); put_float(a + 28, 1.0f);
        put_float(b + 16, 0.5f); put_float(b + 20, 0.25f);
        put_float(b + 24, 1.0f); put_float(b + 28, 0.75f);
        check_true("dynamic_structure_hash",
            rsx_fp_program_structure_hash(a, sizeof(a)) ==
            rsx_fp_program_structure_hash(b, sizeof(b)));
        float constants[4] = {0};
        check_true("dynamic_extract_count",
            rsx_fp_extract_constants(b, sizeof(b), constants, 1) == 1);
