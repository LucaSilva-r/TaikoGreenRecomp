...
static uint32_t http_guest_ea(const void* pointer)
{
    return (uint32_t)(uintptr_t)pointer;
}

static const char* http_guest_string(const char* pointer)
{
    uint32_t ea = http_guest_ea(pointer);
    return ea ? (const char*)vm_translate(ea) : NULL;
}
...
s32 cellHttpCreateClient(CellHttpClientId* clientId)
{
    ...
    for (u32 i = 0; i < CELL_HTTP_MAX_CLIENTS; i++) {
        if (!s_clients[i].in_use) {
            ...
            vm_write32(http_guest_ea(clientId), i);
                              ...
                              const char* method, const CellHttpUri* uri)
{
    ...
    for (u32 i = 0; i < CELL_HTTP_MAX_TRANSACTIONS; i++) {
        if (!s_transactions[i].in_use) {
            ...
            const char* host_method = http_guest_string(method);
            uint32_t uri_ea = http_guest_ea(uri);
            const char* scheme = http_guest_string(
            ...
            vm_write32(http_guest_ea(transId), i);
            printf("[cellHttp] CreateTransaction(client=%u, %s %s) -> trans=%u\n",
                   clientId, host_method, t->url, i);
            return CELL_OK;
        }
    }
                        ...
                        u32* sent)
{
    ...
    if (sent)
        vm_write32(http_guest_ea(sent), size);
                         ...
                         u32* received)
{
    ...
    if (t->body_overflow && t->body_overflow_len > 0) {
        ...
        if (copy > size)
            copy = size;
        memcpy(vm_translate(http_guest_ea(buf)), t->body_overflow, copy);
        filled += copy;
    ...
    if (received)
        vm_write32(http_guest_ea(received), filled);
                             ...
                             const char* value)
{
    ...
    const char* host_name = http_guest_string(name);
    const char* host_value = http_guest_string(value);
    strncpy(h->name, host_name, sizeof(h->name) - 1);
    h->name[sizeof(h->name) - 1] = '\0';
    strncpy(h->value, host_value, sizeof(h->value) - 1);
