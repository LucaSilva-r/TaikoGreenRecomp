...
s32 cellSysCacheMount(char* cachePath)
{
    ...
    /* HLE arguments preserve guest effective addresses.  Writing through the
     * C pointer only happened to survive in configurations that mapped guest
     * memory at low addresses; the native Linux VM is deliberately based at
     * an arbitrary 64-bit address. */
    uint32_t cache_ea = (uint32_t)(uintptr_t)cachePath;
    uint32_t i = 0;
    for (; s_cache_path[i] && i < CELL_SYSCACHE_PATH_MAX - 1; i++)
        vm_write8(cache_ea + i, (uint8_t)s_cache_path[i]);
    vm_write8(cache_ea + i, 0);
    s_cache_mounted = 1;
...
s32 cellDiscGameGetBootDiscInfo(u32* type, char* titleId, u32 titleIdSize)
{
    ...
    if (type)
        vm_write32((uint32_t)(uintptr_t)type,
                   CELL_DISCGAME_TYPE_HDD); /* pretend HDD game */
    ...
    if (titleId && titleIdSize > 0) {
        ...
        const char* value = tid && tid[0] ? tid : "GAME00000";
        uint32_t title_ea = (uint32_t)(uintptr_t)titleId;
        u32 i = 0;
        for (; value[i] && i < titleIdSize - 1; i++)
            vm_write8(title_ea + i, (uint8_t)value[i]);
        vm_write8(title_ea + i, 0);
    }
