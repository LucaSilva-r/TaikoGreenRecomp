                           ...
                           u32 mode)
{
    ...
    for (;;) {
        ...
        /* The old 4 x 50 ms grace charged every rendered frame 200 ms and made
         * Taiko's hardware-check loop appear hung at ~5 FPS.  Even a 2 ms grace
         * reduced attract from ~52 to ~46 FPS.  With the fair guest locking in
         * place, zero-delay force-completion has completed repeated full boots
         * without starving the loader workers, so it is now the default.  The
         * environment override remains useful for scheduling diagnostics. */
        static int wait_ms = -1;
        if (wait_ms < 0) {
            const char* value = getenv("CELLSPURS_WAIT_MS");
            wait_ms = value ? atoi(value) : 0;
            if (wait_ms < 0) wait_ms = 0;
            if (wait_ms > 100) wait_ms = 100;
        }
        if ((wait_ms == 0 || !ef_wait_timed(sync, (uint32_t)wait_ms)) &&
            ++timeouts >= 1) {
            /* This is expected on every frame until SPU execution exists.  A
             * per-frame fprintf serializes all guest threads on stderr and can
             * become part of the frame budget, so show a bounded breadcrumb by
             * default and reserve the flood for an explicit diagnostic trace. */
            static unsigned forced_logs = 0;
            static int spurs_trace = -1;
            if (spurs_trace < 0) spurs_trace = getenv("CELLSPURS_TRACE") ? 1 : 0;
            if (spurs_trace || forced_logs++ < 8)
...
/* A host executor may claim job chains that the SPU would otherwise run: the
 * runtime has no SPU job-chain kernel, so a title that implements its jobs
 * natively registers itself here and drains the chain's ring itself. */
void (*g_spurs_job_chain_runner)(u32 jobChain) = NULL;

/* Pull-side companion: a consumer that must not race the executor calls this to
 * flush whatever the chain still owes. */
void (*g_spurs_job_chain_drain)(void) = NULL;

/* Optional title-side executor for a job descriptor at the instant the guest
 * publishes it into a running chain.  Delaying native execution until an RSX
 * drain is too late for titles which immediately recycle descriptor and DMA
 * input storage. */
void (*g_spurs_job_submit)(u32 job) = NULL;

/* Optional title-side diagnostic: map a guest vertex-output address back to
 * the native job that last produced it and compare completion-time contents
 * with what the renderer is about to consume. */
void (*g_spurs_job_output_probe)(u32 ea) = NULL;

/* cellSpursRunJobChain — start a job chain execution */
s32 cellSpursRunJobChain(void* spurs, void* jobChain)
{
    (void)spurs;
    /* Diagnostic: the graphics vertex-conversion jobs are submitted through a
     * chain we never execute, so dump the descriptor and the caller once. The
     * job-chain command list holds the EA of the SPU job binary we need. */
    {
        extern uint8_t* vm_base;
        u32 jc = (u32)(uintptr_t)jobChain;
        fprintf(stderr, "[jobchain] RunJobChain spurs=0x%08X jobChain=0x%08X\n",
                (u32)(uintptr_t)spurs, jc);
        if (jc >= 0x10000 && vm_base) {
            for (u32 r = 0; r < 8; r++) {
                fprintf(stderr, "[jobchain] +0x%02X:", r * 16);
                for (u32 i = 0; i < 16; i += 4) {
                    u32 w; memcpy(&w, vm_base + jc + r * 16 + i, 4);
                    fprintf(stderr, " %08X", __builtin_bswap32(w));
                }
                fputc('\n', stderr);
            }
        }
    }
    if (g_spurs_job_chain_runner) {
        g_spurs_job_chain_runner((u32)(uintptr_t)jobChain);
        return CELL_OK;
    }
