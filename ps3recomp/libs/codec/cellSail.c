...
{
    (void)mediaInfo;
    const u32 uri_ea = (u32)(uintptr_t)uri;
    const u32 desc_ea = (u32)(uintptr_t)desc;
    char guest_uri[sizeof(s_descs[0].uri)] = {0};
    size_t uri_len = 0;
    if (uri_ea) {
        for (; uri_len + 1 < sizeof(guest_uri); ++uri_len) {
            guest_uri[uri_len] = (char)vm_read8(uri_ea + (u32)uri_len);
            if (!guest_uri[uri_len]) break;
        }
    }
    guest_uri[uri_len] = '\0';
    printf("[cellSail] CreateDescriptor(player=%u, type=%d, uri=\"%s\")\n", handle,
           streamType, uri_ea ? guest_uri : "null");
    if (!desc_ea) return (s32)CELL_SAIL_ERROR_INVALID_ARGUMENT;
    ...
    for (int i = 0; i < MAX_DESCRIPTORS; i++) {
        if (!s_descs[i].in_use) {
            ...
            memcpy(s_descs[i].uri, guest_uri, sizeof(guest_uri));
            vm_write32(desc_ea, (u32)i);
            return CELL_OK;
        }
    }
    return (s32)CELL_SAIL_ERROR_OUT_OF_MEMORY;
...
{
    const u32 type_ea = (u32)(uintptr_t)type;
    if (desc >= MAX_DESCRIPTORS || !s_descs[desc].in_use || !type_ea)
        return (s32)CELL_SAIL_ERROR_INVALID_ARGUMENT;
    vm_write32(type_ea, (u32)s_descs[desc].streamType);
    return CELL_OK;
}
...
{
    const u32 uri_ea = (u32)(uintptr_t)uri;
    if (desc >= MAX_DESCRIPTORS || !s_descs[desc].in_use || !uri_ea || !maxLen)
        return (s32)CELL_SAIL_ERROR_INVALID_ARGUMENT;
    u32 i = 0;
    for (; i + 1 < maxLen && s_descs[desc].uri[i]; ++i)
        vm_write8(uri_ea + i, (u8)s_descs[desc].uri[i]);
    vm_write8(uri_ea + i, 0);
    return CELL_OK;
}
